...
/* =========================================================
   VELOUR STORY ENGINE V4.4.9 — LIFESTYLE WORLD PACK
...
(() => {
  ...
  function installUI(){
    ...
    p.innerHTML=`
      <div class="v40-title"><b>✦ STORY ENGINE V4.4.9 · LIFESTYLE WORLD PACK</b><span class="v40-badge">기존 저장함 호환</span></div>
  ...
  function syncUI(full=true){
    ...
    const s=p.querySelector('#v4Status'); if(s){
      ...
      s.innerHTML=`<b>V4.4.9 ACTIVE</b> · ${esc(worldLabel())} · ${esc(relationshipLabel())} → ${esc(trajectoryLabel())}<br>`+
...
- 메타는 앱에서 자동 제거되므로 독자에게 설명하지 않는다.`;
  ...
  if(typeof previousBuild==='function'){
    window.buildPrompt=function(isContinue=false){
      ...
      return `${base}\n\n===== VELOUR STORY ENGINE V4.4.9 · LIFESTYLE + POST-UNLOCK + REFUSAL HOTFIX OVERRIDE =====
[우선순위] 사용자 현재 입력 > V4.4.9 HARD CANON/스토리라인 잠금 > V4.4.9 생활밀착/외형/장기 메모리 > 기존 자동 디렉터.
- 위쪽 기존 프롬프트에 남아 있을 수 있는 옛 ‘배경 세계관/서사 단계/관계성/2~4화 페이싱’ 값은 레거시 호환 정보일 뿐이다. 충돌하면 아래 V4.4.9 값만 따른다.
- 기존 V3.5 관계 태그는 V4.4.9 관계축과 중복되므로 이번 프롬프트에서는 비활성화했다.
...
===== END VELOUR V4.4.9 =====`;
  ...
  function patchDraft(){
    try{ const d=safeParse(localStorage.getItem(DRAFT_KEY)||'null',null); if(!d)return; d.v4State=clone(state); if(d.currentText)d.currentText=stripMetaText(d.currentText); if(d.storyHistory)d.storyHistory=stripMetaText(d.storyHistory); if(Array.isArray(d.episodes)) d.episodes=d.episodes.map(ep=>Object.assign({},ep,{text:stripMetaText(ep.text||'')})); localStorage.setItem(DRAFT_KEY,JSON.stringify(d)); }catch(e){}
  }
  ...
  if(typeof previousGenerate==='function'){
    window.generateStory=async function(isContinue=false){
      ...
      if(reason){
        ...
        if(!secondReason){ updateMemory(parsed.meta,secondEp); } else { console.warn('VELOUR V4.4.9: retry still violated pacing; memory not committed', secondReason); }
  ...
  stripMetaEverywhere();
  patchDraft();
  console.info('✦ VELOUR Story Engine V4.4.9 Lifestyle World Pack loaded');
})();
