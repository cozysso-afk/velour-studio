...
(() => {
  ...
  function parseMeta(raw){const src=String(raw||''),open='[[VELOUR_V4_META]]',close='[[/VELOUR_V4_META]]';const a=src.indexOf(open);if(a<0)return null;const b=src.indexOf(close,a+open.length);const chunk=(b>=0?src.slice(a+open.length,b):src.slice(a+open.length)).trim();try{return JSON.parse(chunk);}catch(_){return null;}}
  ...
  async function acceptVelourVaultResponse(id){
    ...
    if(!isAcceptableRecord(r))return alert(`이 응답은 현재 이어쓰기 위치에 붙일 수 없어.\n현재 확정: EP.${confirmedEpisode()}\n선택 응답: EP.${ep||'-'}`);
    const clean=String(r.readerText||'').trim();if(!confirm(`EP.${String(ep).padStart(2,'0')} 미확정 응답을 정식 본문으로 채택할까?\n\n✓ 새 Gemini 요청 없음\n✓ 다음 화는 EP.${ep+1}부터 진행`))return;
    try{
      let history='';try{history=String(storyHistory||'');}catch(_){}const normalized=history.replace(/\s+/g,' ').trim(),tail=clean.slice(-320).replace(/\s+/g,' ').trim();if(!normalized||!tail||!normalized.includes(tail))history=history.trim()?history.trimEnd()+'\n\n'+clean:clean;try{storyHistory=history;}catch(_){}try{episodeCount=ep;}catch(_){}
      const meta=guardedMeta(parseMeta(r.rawText||'')),acceptedDirection=String(document.getElementById('v33Next')?.value||'').trim();qa.rememberConfirmedEpisode?.(ep,false);qa.clearPendingRetryEpisode?.();if(meta)qa.updateMemory?.(meta,ep,acceptedDirection);
      const novel=document.getElementById('novelText'),title=document.getElementById('resultTitle'),panel=document.getElementById('resultPanel'),counter=document.getElementById('v35CharCount'),next=document.getElementById('btnNext'),dir=document.getElementById('v33Next');if(novel)novel.innerText=clean;if(title)title.innerText=`EPISODE ${String(ep).padStart(2,'0')}`;if(panel)panel.style.display='block';if(next)next.style.display='block';if(dir)dir.value='';try{localStorage.removeItem('VELOUR_NEXT_DIRECTIVE_V33');}catch(_){}document.getElementById('velourHardLockReviewNotice')?.remove();const actions=document.getElementById('velourGenerationDiagnosticActions');if(actions)actions.style.display='none';qa.markGenerationOutcome?.('committed',{episode:ep,attemptedEpisode:ep,userAcceptedVault:true,responseVaultId:String(r.id||id)});if(counter)counter.textContent=`본문 ${clean.length.toLocaleString()}자 · 사용자 채택 확정`;try{await window.__VELOUR_IDB_SAVE_DRAFT__?.();}catch(_){}try{await window.__VELOUR_IDB_PATCH_DRAFT_V4__?.(window.__VELOUR_V4_STATE_SNAPSHOT__?.());}catch(_){}const modal=document.getElementById('velourResponseVaultModal');if(modal)modal.style.display='none';panel?.scrollIntoView({behavior:'smooth',block:'start'});alert(`✓ EP.${String(ep).padStart(2,'0')}로 확정했어.`);
    }catch(e){console.error('VELOUR vault accept failed',e);alert('응답 채택 중 오류: '+String(e?.message||e));}
  }
  window.acceptVelourVaultResponse=acceptVelourVaultResponse;
  const originalShowVault=window.showVelourResponseVault;if(typeof originalShowVault==='function'){window.showVelourResponseVault=async function(){await originalShowVault.apply(this,arguments);const list=document.getElementById('velourVaultList');if(!list)return;let rows=[];try{rows=await qa.responseVaultAll(RESPONSE_STORE);}catch(_){return;}rows.sort((a,b)=>String(b.receivedAt||'').localeCompare(String(a.receivedAt||'')));const cards=Array.from(list.children);rows.forEach((r,i)=>{const card=cards[i];if(!card||!isAcceptableRecord(r))return;const actions=card.lastElementChild;if(!actions||actions.querySelector('[data-vault-accept]'))return;const btn=document.createElement('button');btn.type='button';btn.dataset.vaultAccept='1';btn.textContent='✓ 이 응답으로 확정';btn.style.cssText='flex:1;border:1px solid rgba(126,224,160,.35);background:rgba(90,200,130,.10);color:#bff4cf;border-radius:8px;padding:7px;font-size:9.5px;font-weight:800';btn.onclick=()=>acceptVelourVaultResponse(r.id);actions.prepend(btn);});};}
  const observer=new MutationObserver(()=>{const note=document.getElementById('velourHardLockReviewNotice');if(!note||note.querySelector('[data-vault-accept-review]')||!pendingEpisode())return;const btn=document.createElement('button');btn.type='button';btn.dataset.vaultAcceptReview='1';btn.textContent='✓ 현재 미확정 응답 채택';btn.style.cssText='margin-top:10px;width:100%;border:1px solid rgba(126,224,160,.35);background:rgba(90,200,130,.11);color:#bff4cf;border-radius:9px;padding:8px 10px;font-size:10px;font-weight:800';btn.onclick=async()=>{let rows=[];try{rows=await qa.responseVaultAll(RESPONSE_STORE);}catch(_){return;}rows=rows.filter(isAcceptableRecord).sort((a,b)=>String(b.receivedAt||'').localeCompare(String(a.receivedAt||'')));if(rows.length===1)return acceptVelourVaultResponse(rows[0].id);window.showVelourResponseVault?.();};note.appendChild(btn);});observer.observe(document.body,{subtree:true,childList:true});
...
__velourLoadHotfix('__VELOUR_CONTINUITY_COST_LOADER__','./velour-v4.4.38-continuity-hotfix.js?v=1');
__velourLoadHotfix('__VELOUR_CONTINUITY_VAULT_EDGE_LOADER__','./velour-v4.4.38-continuity-vault-hotfix.js?v=2');
...
__velourLoadHotfix('__VELOUR_PROSE_PREFERENCE_LOADER__','./velour-v4.4.38-prose-preference-hotfix.js?v=1');
