...
/* VELOUR V4.4.38 — Response Vault acceptance hotfix
   ...
   Narrow hotfix policy:
   - preserve vault acceptance and canon prompt stabilization
   - age-stage detection alone is advisory and must not leave an otherwise valid episode unconfirmed
   - no automatic Gemini retry and no new content filter
*/
(() => {
  'use strict';
  ...
  function parseMeta(raw){
    const src=String(raw||''),open='[[VELOUR_V4_META]]',close='[[/VELOUR_V4_META]]';
    const a=src.indexOf(open); if(a<0)return null;
  ...
  function isAcceptableRecord(r){
    ...
    const pending=pendingEpisode(),confirmed=confirmedEpisode();
  ...
  function guardedMeta(meta){
    ...
    // ageStageViolation is deliberately advisory here: the legacy age parser can
    // misread a past school-age mention as the current canon stage.
  ...
  async function acceptVelourVaultResponse(id,options={}){
    let r=null;
    try{r=await qa.responseVaultGet(RESPONSE_STORE,String(id))}catch(e){if(!options.silent)return alert('응답 금고를 읽지 못했어: '+String(e?.message||e));return false}
    if(!r){if(!options.silent)alert('응답 금고에서 해당 본문을 찾지 못했어.');return false}
    const ep=nEp(r.attemptedEpisode);
    if(!isAcceptableRecord(r)){
      if(!options.silent){const pending=pendingEpisode(),confirmed=confirmedEpisode();alert(`이 응답은 현재 이어쓰기 위치에 안전하게 붙일 수 없어.\n\n현재 확정: EP.${confirmed}\n현재 재시도 대기: ${pending?'EP.'+pending:'없음'}\n선택 응답: EP.${ep||'-'}`)}
      return false;
    }
    const clean=String(r.readerText||'').trim();
    if(!options.silent&&!confirm(`EP.${String(ep).padStart(2,'0')}의 미확정 응답을 정식 본문으로 채택할까?\n\n✓ 새 Gemini 요청 없음\n✓ 현재 본문을 EP.${ep}로 확정\n✓ 다음 화는 EP.${ep+1}부터 진행\n✓ 응답 금고 원문은 그대로 보존`))return false;
    try{
      let history='';try{history=String(storyHistory||'')}catch(_){}
      const normalized=history.replace(/\s+/g,' ').trim(),tail=clean.slice(-320).replace(/\s+/g,' ').trim();
      ...
      qa.rememberConfirmedEpisode?.(ep,false); qa.clearPendingRetryEpisode?.(); if(meta)qa.updateMemory?.(meta,ep);
      ...
      document.getElementById('velourHardLockReviewNotice')?.remove();const diag=document.getElementById('velourGenerationDiagnosticActions');if(diag)diag.style.display='none';
      qa.markGenerationOutcome?.('committed',{episode:ep,attemptedEpisode:ep,userAcceptedVault:!options.silent,ageStageAdvisoryAutoCommit:!!options.ageStageOnly,responseVaultId:String(r.id||id)});
      if(counter){counter.style.color='#bca7b2';counter.textContent=`본문 ${clean.length.toLocaleString()}자 · 확정 · ENGINE V4.4.38`;}
      try{await window.__VELOUR_IDB_SAVE_DRAFT__?.()}catch(e){console.warn('VELOUR vault accept: draft save warning',e)}
      ...
      return true;
    }catch(e){console.error('VELOUR vault response accept failed',e);if(!options.silent)alert('응답 채택 중 오류가 났어: '+String(e?.message||e));return false}
  }
  ...
  if(typeof originalShowVault==='function'){
    window.showVelourResponseVault=async function(){
      await originalShowVault.apply(this,arguments);const list=document.getElementById('velourVaultList');if(!list)return;
      let rows=[];try{rows=await qa.responseVaultAll(RESPONSE_STORE)}catch(_){return}
      rows.sort((a,b)=>String(b.receivedAt||'').localeCompare(String(a.receivedAt||'')));const cards=Array.from(list.children);
      rows.forEach((r,i)=>{const card=cards[i];if(!card||!isAcceptableRecord(r))return;const actions=card.lastElementChild;if(!actions||actions.querySelector('[data-vault-accept]'))return;const btn=document.createElement('button');btn.type='button';btn.dataset.vaultAccept='1';btn.textContent='✓ 이 응답으로 확정';btn.style.cssText='flex:1;border:1px solid rgba(126,224,160,.35);background:rgba(90,200,130,.10);color:#bff4cf;border-radius:8px;padding:7px;font-size:9.5px;font-weight:800';btn.onclick=()=>acceptVelourVaultResponse(r.id);actions.prepend(btn);});
    };
  }

  function ageStageOnlyReview(note){
    const s=String(note?.textContent||'');
    const age=/(?:CANON 연령 단계|연령\/학년 HARD LOCK|다음 연령\/학년 단계|연령\/학년 단계를 임의로)/i.test(s);
    const other=/(?:거주 HARD LOCK|HARD CANON 위반|현재 CANON 단계 건너뜀|미래 단계 선행 실행|본문이 너무 짧|출력 미완료|금지된|professional|expression)/i.test(s);
    return age&&!other;
  }

  let ageAutoCommitBusy=false;
  const observer=new MutationObserver(async()=>{
    const note=document.getElementById('velourHardLockReviewNotice');if(!note)return;
    // Narrow fix: if the ONLY blocking reason is the legacy age-stage detector,
    // promote the already-received candidate without another API request.
    if(ageStageOnlyReview(note)&&!ageAutoCommitBusy){
      ageAutoCommitBusy=true;
      try{
        let rows=await qa.responseVaultAll(RESPONSE_STORE);
        rows=rows.filter(r=>isAcceptableRecord(r)).sort((a,b)=>String(b.receivedAt||'').localeCompare(String(a.receivedAt||'')));
        if(rows.length)await acceptVelourVaultResponse(rows[0].id,{silent:true,ageStageOnly:true});
      }catch(e){console.warn('VELOUR age-stage advisory auto-commit warning',e)}finally{ageAutoCommitBusy=false}
      return;
    }
    if(note.querySelector('[data-vault-accept-review]'))return;const ep=pendingEpisode();if(!ep)return;
    const box=document.createElement('div');box.style.cssText='display:flex;gap:7px;margin-top:10px;flex-wrap:wrap';const btn=document.createElement('button');btn.type='button';btn.dataset.vaultAcceptReview='1';btn.textContent='✓ 현재 미확정 응답 채택';btn.style.cssText='flex:1;border:1px solid rgba(126,224,160,.35);background:rgba(90,200,130,.11);color:#bff4cf;border-radius:9px;padding:8px 10px;font-size:10px;font-weight:800';btn.onclick=async()=>{let rows=[];try{rows=await qa.responseVaultAll(RESPONSE_STORE)}catch(_){return alert('응답 금고를 읽지 못했어.')}rows=rows.filter(r=>isAcceptableRecord(r)).sort((a,b)=>String(b.receivedAt||'').localeCompare(String(a.receivedAt||'')));if(!rows.length)return window.showVelourResponseVault?.();if(rows.length===1)return acceptVelourVaultResponse(rows[0].id);window.showVelourResponseVault?.();};box.appendChild(btn);note.appendChild(box);
  });
  ...
  if(!window.__VELOUR_CANON_PROMPT_STABILIZER__){
    ...
    const explicitCanonStage=(text,max)=>{const s=String(text||'');const patterns=[/(?:캐논|CANON)(?:\s*스토리라인)?[^\d]{0,18}(\d+)\s*단계/i,/(?:스토리라인|storyline)[^\d]{0,18}(\d+)\s*단계/i,/(\d+)\s*단계\s*(?:캐논|CANON)/i];for(const rx of patterns){const m=s.match(rx),n=Number(m?.[1]||0);if(n>=1&&n<=max)return n-1}return -1};
    const removeAgeHardLock=prompt=>{const lines=String(prompt||'').split('\n'),out=[];let skipping=false;for(const line of lines){if(/^\[AGE\/TIMELINE HARD LOCK\b/i.test(line.trim())){skipping=true;continue}if(skipping){if(!line.trim()){skipping=false;continue}if(/^\[[^\]]+\]/.test(line.trim())){skipping=false;out.push(line)}continue}out.push(line)}return out.join('\n')};
    ...
    if(typeof priorBuildPrompt==='function')window.buildPrompt=function(isContinue=false){
      ...
      const anchor=['[CURRENT CANON PRIORITY — 생성 방향 정리]',userNext?`- 사용자 다음 화 지시: ${userNext}`:'',currentBeat?`- 이번 화의 현재 CANON 단계: ${effectiveIndex+1}/${beats.length} · ${currentBeat}`:'',requestedIndex>=0?'- 사용자가 이번 요청에서 단계 번호를 직접 지정했으므로 저장된 진행 커서보다 이 요청의 단계 지정을 우선한다.':'','- 최근 장면 메모·열린 떡밥·이전 화의 자생 사건은 현재 CANON 단계와 사용자 지시를 보조하는 참고자료다. 충돌하면 현재 CANON 단계와 사용자 지시를 따른다.','- 과거 시절 언급이나 회상은 현재 시점을 자동으로 되돌리는 근거로 사용하지 않는다. 현재 시점은 이번 요청과 현재 CANON 단계의 문맥으로 판단한다.','- 이 지시는 생성 방향을 정리하기 위한 것이며 본문을 검열·차단하거나 자동 재생성을 요구하지 않는다.'].filter(Boolean).join('\n');
      prompt=`${anchor}\n\n${prompt}`.trim();window.__VELOUR_CANON_PROMPT_STABILIZER_LAST__={beforeChars,afterChars:prompt.length,savedChars:Math.max(0,beforeChars-prompt.length),storedBeatIndex:storedIndex,effectiveBeatIndex:effectiveIndex,explicitBeatOverride:requestedIndex>=0,currentBeat,userDirectionPresent:!!userNext,at:new Date().toISOString()};return prompt;
    };
  }
  console.info('✦ VELOUR V4.4.38 Response Vault + canon/age advisory hotfix loaded');
})();