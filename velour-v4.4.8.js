...
/* =========================================================
   VELOUR STORY ENGINE V4.4.8 — POST-UNLOCK RELATIONSHIP PACE PATCH
...
- 메타는 앱에서 자동 제거되므로 독자에게 설명하지 않는다.`;
  ...
  if(typeof previousBuild==='function'){
    window.buildPrompt=function(isContinue=false){
      ...
      return `${base}\n\n===== VELOUR STORY ENGINE V4.4.8 · POST-UNLOCK PACE + REFUSAL HOTFIX OVERRIDE =====
[우선순위] 사용자 현재 입력 > V4.4.8 HARD CANON/스토리라인 잠금 > V4.4.8 외형·장기 메모리 > 기존 자동 디렉터.
- 위쪽 기존 프롬프트에 남아 있을 수 있는 옛 ‘배경 세계관/서사 단계/관계성/2~4화 페이싱’ 값은 레거시 호환 정보일 뿐이다. 충돌하면 아래 V4.4.8 값만 따른다.
- 기존 V3.5 관계 태그는 V4.4.8 관계축과 중복되므로 이번 프롬프트에서는 비활성화했다.
...
===== END VELOUR V4.4.8 =====`;
  ...
  stripMetaEverywhere();
  patchDraft();
  console.info('✦ VELOUR Story Engine V4.4.8 Post-Unlock Pace loaded');
})();
