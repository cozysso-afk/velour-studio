...
(() => {
  ...
  window.__VELOUR_QUALITY_RESTORE_VERSION__ = '1.3.3';
  ...
  function qualityDirective(s){
    ...
    return `\n[PROSE QUALITY RESTORE — CANON을 바꾸지 않는 문체 품질 지시]\n- HARD CANON, 현재 CANON STORYLINE 단계, 이번 화 사용자 지시가 항상 우선이다. 이 블록은 사건·연령·직업·관계를 새로 만들지 않는다.\n- 신체 설정값은 외형 일관성을 위한 내부 참고값이다. ${exact ? '이번 화 사용자가 정확한 수치 언급을 직접 요구했으므로 필요한 범위에서만 수치를 사용할 수 있다.' : '저장된 컵 문자·cm·정확한 신체 치수와 고정 체형 설명은 본문에 재소개하지 않는다. 설정값을 실루엣·비율·몸매 특징 문장으로 바꿔 말하는 것도 반복 설명이면 금지한다.'}\n- 신체 묘사 풍부도=${richness}는 서술자가 인물의 고정 외형을 반복 설명하라는 뜻이 아니다. 친밀감이나 욕망이 활성화된 장면에서는 상대가 상대를 보고 느끼며 하는 직접적인 외형 반응 대사를 우선하고, 비친밀 장면에서는 외형 언급을 의무적으로 끼워 넣지 않는다.\n- 특히 본문 중간에 서사를 멈추고 “그는 원래 ~한 체형이었다”, “그녀는 ~한 몸매를 지녔다”, “그의/그녀의 특징은 ~였다”처럼 인물 소개문·설정 해설문을 삽입하지 않는다. 독자가 이미 아는 고정 외형을 재설명하는 문단은 0개가 기본값이다.\n- 사용자가 이번 화 지시에서 특정 표현·어휘·말투·직접성의 수준을 직접 제시했다면 그 의미와 강도를 임의로 점잖게 만들거나 일반적인 칭찬으로 치환하지 않는다. 사용자가 정한 레지스터를 유지하되, 같은 문장을 기계적으로 복붙하지 말고 캐릭터 성격과 현재 반응에 맞게 자연스럽게 변주한다.\n- 외형 반응 대사는 설정 낭독이 아니다. “원래 이런 몸이다”를 설명하는 대신 지금 상대를 보고 느끼는 즉각적인 감탄·질문·도발·반응으로 쓴다. 서술자는 그 대사를 받쳐주는 시선·표정·움직임만 최소한으로 사용한다.\n- 키스 밀도=${kissing}, 애무 길이=${foreplay}, 장면 중 지속 애무=${caress}. 키스와 애무를 짧은 통과 의례처럼 처리하지 않는다.\n${kissingRule}\n${foreplayRule}\n- 이번 화 사용자 지시에 ‘애무/전희/키스를 길게·오래·충분히’가 들어 있으면 UI 선택값보다 그 직접 지시를 우선한다.\n- 더티톡 강도=${dirty}/100, 빈도=${dirtyFrequency}/100. 강도는 직접성, 빈도는 등장 성향이다. 빈도를 높여도 같은 의미군을 반복해서 채우지 말고 관계·감정·상황에 맞춰 어휘, 문장 길이, 질문/칭찬/도발/반응형 대사를 다양하게 바꾼다. 캐릭터 말투와 성격은 유지한다. 사용자가 직접 정한 대사 레지스터가 있으면 더티톡 강도 수치보다 그 직접 지시를 우선한다.\n${blockingRule}\n- 구도 전환은 장면을 산만하게 만들기 위한 것이 아니다. 한 구도가 충분히 살아난 뒤 공간·감정·상대 반응이 다음 움직임을 만들게 하며, 최근 화와 똑같은 시작→진행 패턴을 반복하지 않는다.\n- LONG 선택은 실제 장면 배분을 늘리라는 뜻이다. 단, 같은 묘사를 반복해서 분량만 부풀리지 말고 서로 다른 반응과 감정 변화로 밀도를 높인다.`;
  }
