...
/* =========================================================
   VELOUR STORY ENGINE V4.4.11 — STORAGE + INTIMACY VARIETY UNIFIED
...
(() => {
  ...
  const DEFAULT = {
    ...
    runtime:{timeline:[],openThreads:[],scenes:[],relationshipState:'',lastAdultEpisode:0,retryCount:0,positionUsage:{},lastSuggestedPositions:[]}
  };
  ...
  function installUI(){
    ...
    p.innerHTML=`
      <div class="v40-title"><b>✦ STORY ENGINE V4.4.11 · STORAGE + VARIETY</b><span class="v40-badge">기존 저장함 호환</span></div>
      ...
      <details class="v40-section" id="v41SecPacing">
        ...
        <div class="v40-note">V4.4.11은 위 큰 구도 선택 안에서 <b>${POSITION_CATALOG.length}종 세부 체위/배치</b>를 자동 로테이션해. 매회 전체 목록을 프롬프트에 쏟지 않고, 최근 미사용·저사용 후보 4개만 골라 AI가 장소/감정선에 맞는 하나를 쓰게 해서 반복과 프롬프트 과밀을 같이 줄여.</div>
      </details>
  ...
  function syncUI(full=true){
    ...
    const s=p.querySelector('#v4Status'); if(s){
      ...
      s.innerHTML=`<b>V4.4.11 ACTIVE</b> · ${esc(worldLabel())} · ${esc(relationshipLabel())} → ${esc(trajectoryLabel())}<br>`+
        ...
        `장기 메모리 ${state.runtime.timeline.length}건 · 미회수 ${state.runtime.openThreads.length}건 · 장면기록 ${state.runtime.scenes.length}건 · 세부 체위풀 ${POSITION_CATALOG.length}종${last?.position?` · 최근 ${esc(last.position)}`:(last?.pattern?` · 최근 ${esc(last.pattern)}`:'')}`;
    }
  }
...
${scenes.length?`최근 장면 지문:\n${scenes.map(s=>`- EP${s.episode||'?'} | 장소 ${s.location||'?'} | 목적 ${s.purpose||'?'} | 큰구도 ${s.pattern||'없음'} | 세부체위 ${s.position||'없음'} | 시작 ${s.initiation||'?'} | 주도 ${s.control||'?'} | 대화 ${s.dialogueTone||'?'} | 엔딩 ${s.ending||'?'}`).join('\n')}`:'최근 장면 지문: 없음.'}
...
- “술/집/묘한 분위기/직전에서 끊기” 같은 동일 구조를 이름만 바꿔 반복하지 않는다.`;
  ...
  function varietyDirective(){
    ...
    save(state);
    return `
[성인 장면 다양성 · V4.4.11 AUTO ROTATION]
...
- 사용자가 이번 화에 특정 체위/구도를 직접 지시하면 그 지시가 최우선이다.`;
  }
...
- 메타는 앱에서 자동 제거되므로 독자에게 설명하지 않는다.`;
  ...
  if(typeof previousBuild==='function'){
    window.buildPrompt=function(isContinue=false){
      ...
      return `${base}\n\n===== VELOUR STORY ENGINE V4.4.11 · STORAGE + VARIETY + LIFESTYLE OVERRIDE =====
[우선순위] 사용자 현재 입력 > V4.4.11 HARD CANON/스토리라인 잠금 > V4.4.11 생활밀착/외형/장기 메모리 > 기존 자동 디렉터.
- 위쪽 기존 프롬프트에 남아 있을 수 있는 옛 ‘배경 세계관/서사 단계/관계성/2~4화 페이싱’ 값은 레거시 호환 정보일 뿐이다. 충돌하면 아래 V4.4.11 값만 따른다.
- 기존 V3.5 관계 태그는 V4.4.11 관계축과 중복되므로 이번 프롬프트에서는 비활성화했다.
...
===== END VELOUR V4.4.11 =====`;
  ...
  function updateMemory(meta,ep){
    ...
    state.runtime.scenes.push({episode:ep,location:String(meta.location||'').trim(),purpose:String(meta.purpose||'').trim(),pattern:String(meta.pattern||'none').trim(),positionId:canonicalPosId,position:canonicalPosName,initiation:String(meta.initiation||'').trim(),control:String(meta.control||'').trim(),dialogueTone:String(meta.dialogueTone||'').trim(),ending:String(meta.ending||'').trim(),adultScene:!!meta.adultScene});
  ...
  if(typeof previousGenerate==='function'){
    window.generateStory=async function(isContinue=false){
      ...
      if(reason){
        ...
        if(!secondReason){ updateMemory(parsed.meta,secondEp); } else { console.warn('VELOUR V4.4.11: retry still violated pacing/variety; memory not committed', secondReason); }
  ...
  /* =========================================================
     V4.4.10 STORAGE ENGINE (bundled into V4.4.11)
  ...
  window.__VELOUR_STORAGE_QA__={idbOpen,idbGet,idbGetAll,idbPut,idbDelete,storageStories,saveDraftIDB,migrateLegacyStorage,positionCandidates,selectedPositionPool,prettyBytes};
  ...
  stripMetaEverywhere();
  patchDraft();
  console.info('✦ VELOUR Story Engine V4.4.11 Storage + Variety Unified loaded');
})();
