...
/* =========================================================
   VELOUR STORY ENGINE V4.4.18 — BODY PROSE VARIATION
...
(() => {
  ...
  function installUI(){
    ...
    p.innerHTML=`
      <div class="v40-title"><b>✦ STORY ENGINE V4.4.18 · CAUSAL BUILDUP LOCK</b><span class="v40-badge">기존 저장함 호환</span></div>
      ...
      <details class="v40-section" id="v41SecPacing">
        ...
        <div class="v40-note">V4.4.18은 위 큰 구도 선택 안에서 <b>${POSITION_CATALOG.length}종 세부 체위/배치</b>를 자동 로테이션해. 매회 전체 목록을 프롬프트에 쏟지 않고, 최근 미사용·저사용 후보 4개만 골라 AI가 장소/감정선에 맞는 하나를 쓰게 해서 반복과 프롬프트 과밀을 같이 줄여.</div>
  ...
  function syncUI(full=true){
    ...
    const s=p.querySelector('#v4Status'); if(s){
      ...
      s.innerHTML=`<b>V4.4.18 BUILDUP LOCK ACTIVE</b> · ${esc(worldLabel())} · ${esc(relationshipLabel())} → ${esc(trajectoryLabel())}<br>`+
...
- 메타는 앱에서 자동 제거되므로 독자에게 설명하지 않는다. 내부 id/영문 키는 본문에 쓰지 않고 메타에도 필요하지 않다.`;
  ...
  if(typeof previousBuild==='function'){
    window.buildPrompt=function(isContinue=false){
      ...
      return `${base}\n\n===== VELOUR STORY ENGINE V4.4.18 · CAUSAL BUILDUP OVERRIDE =====
[우선순위] V4.4.18 HARD LOCK(금지/확정 캐논/현재 실행 단계) > 인과적 빌드업과 직전 확정 상태 > 사용자 현재 화 지시 > 취향·생활밀착·외형·친밀 장면 질감 > 기존 자동 디렉터.
- 설정 UI에서 OFF/금지로 둔 항목과 현재 CANON STORYLINE 단계는 단순 취향 제안이 아니라 HARD LOCK이다. 이번 화 추가 지시보다 우선한다.
- 위쪽 기존 프롬프트에 남아 있을 수 있는 옛 ‘배경 세계관/서사 단계/관계성/2~4화 페이싱’ 값은 레거시 호환 정보일 뿐이다. 충돌하면 아래 V4.4.18 값만 따른다.
- 기존 V3.5 관계 태그는 V4.4.18 관계축과 중복되므로 이번 프롬프트에서는 비활성화했다.
...
===== END VELOUR V4.4.18 =====`;
  ...
  function retryReason(meta,text,ep,isContinue=false,userInstruction=''){
    ...
    if(String(meta?.causalBridge||'').toLowerCase()==='broken' || meta?.setupMissing) reasons.push('사건의 원인·발생 계기·중간 반응이 생략되어 결과/중반부터 시작했다. 원인→반응→선택→결과를 화면 안에서 연결하고, 독자가 보지 못한 핵심 사건을 기정사실로 만들지 말 것.');
    ...
    window.generateStory=async function(isContinue=false){
      ...
      if(reason){
        ...
        if(!secondReason){
          ...
          console.warn('VELOUR V4.4.18: retry still violated HARD LOCK/continuity/pacing; output rejected', secondReason);
          rollbackBase();
          state.runtime.retryDirective=''; state.runtime.retryCount=0; save(state);
...
재생성본도 현재 단계/연속성/금지 언어 설정을 지키지 못해서 이번 화를 저장하지 않았어. 같은 EP에서 다시 생성해줘.`;
  ...
  /* =========================================================
     V4.4.10 STORAGE ENGINE (bundled into V4.4.18)
  ...
  stripMetaEverywhere();
  patchDraft();
  console.info('✦ VELOUR Story Engine V4.4.18 Causal Buildup Lock loaded');
})();
