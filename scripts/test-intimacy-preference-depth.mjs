...
const ensembleQA={
  loadCfg:()=>ensembleCfg,
  activePartners:cfg=>cfg.partners.slice(0,1),
...
const document={
  ...
  createElement(){return {id:'',style:{},dataset:{},parentNode:null,innerHTML:'',querySelectorAll(){return [];},insertAdjacentElement(){}};}
};
...
assert.equal(window.__VELOUR_INTIMACY_PREFERENCE_DEPTH_VERSION__,'1.0.0');
...
const stim=qa.mutualStimCandidates(heroine,partner,cfg,30);
...
assert.ok(compat.shared.some(x=>x.id==='exposure_fantasy'));
assert.ok(compat.warnings.some(x=>x.includes('공공장소')));

const prompt=window.buildPrompt();
assert.match(prompt,/INTIMACY PREFERENCE DEPTH V1/);
assert.match(prompt,/가벼운 S\/M/);
assert.match(prompt,/비동의 제3자 노출/);
...
assert.match(loader,/intimacy-preference-depth\.js\?v=1/);

console.log('PASS: intimacy preference depth keeps mutual OFF rules, complementary S-M dynamics, privacy guards, persistence, and final canon order');
