...
const cfg={
  ...
  heroine:{
    ...
    personalities:['calm','proactive'],tendencies:['initiative','boundary'],initiativeIntensity:85,initiativeFrequency:'high',
...
const document={
  ...
  createElement(){return {style:{},dataset:{},remove(){},querySelectorAll(){return [];},addEventListener(){}};}
};
...
assert.equal(window.__VELOUR_ENSEMBLE_CHARACTER_PREFERENCES_VERSION__,'1.0.0');
...
assert.equal(loaded.heroine.initiativeIntensity,85);
...
assert.equal(qa.initiativeBudget(loaded.heroine),3);
...
console.log('PASS: ensemble character preferences keep 1:1 compatibility, isolate multi-partner edges, persist profiles, and enforce mutual tri-state preferences');
