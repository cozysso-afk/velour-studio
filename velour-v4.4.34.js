...
/* =========================================================
   VELOUR STORY ENGINE V4.4.34 — STAGE-ISOLATED PROMPT + RESPONSE VAULT
...
(() => {
  ...
  window.__VELOUR_ENGINE_VERSION__ = '4.4.34';
  ...
  const MAX_SCENES = 12;
  // V4.4.34 three-tier longform memory.
  ...
  function installUI(){
    ...
    p.innerHTML=`
      <div class="v40-title"><b>✦ STORY ENGINE V4.4.34 · CAUSAL BUILDUP LOCK</b><span class="v40-badge">기존 저장함 호환</span></div>
      ...
      <details class="v40-section" id="v41SecPacing">
        ...
        <div class="v40-note">V4.4.34은 위 큰 구도 선택 안에서 <b>${POSITION_CATALOG.length}종 세부 체위/배치</b>를 자동 로테이션해. 매회 전체 목록을 프롬프트에 쏟지 않고, 최근 미사용·저사용 후보 소수만 골라 AI가 장소/감정선에 맞는 하나를 쓰게 해서 반복과 프롬프트 과밀을 같이 줄여.</div>
  ...
  function syncUI(full=true){
    ...
    const s=p.querySelector('#v4Status'); if(s){
      ...
      s.innerHTML=`<b>V4.4.34 BUILDUP LOCK ACTIVE</b> · ${esc(worldLabel())} · ${esc(relationshipLabel())} → ${esc(trajectoryLabel())}<br>`+
...
- 메타는 앱이 제거하므로 본문에 설명하거나 반복하지 않는다.`;
  ...
  function compactLegacyContext(text,isContinue=false){
    let out=String(text||'');
    // V3.5 and the pinned base each inject raw story history. V4.4.34 already supplies a compact
  ...
  if(typeof previousBuild==='function'){
    window.buildPrompt=function(isContinue=false){
      ...
      const prompt=`${base}\n\n===== VELOUR STORY ENGINE V4.4.34 · CAUSAL BUILDUP OVERRIDE =====
[우선순위] V4.4.34 HARD LOCK(금지/확정 캐논/현재 실행 단계) > 인과적 빌드업과 직전 확정 상태 > 사용자 현재 화 지시 > 취향·생활밀착·외형·친밀 장면 질감 > 기존 자동 디렉터.
- 설정 UI에서 OFF/금지로 둔 항목과 현재 CANON STORYLINE 단계는 단순 취향 제안이 아니라 HARD LOCK이다. 이번 화 추가 지시보다 우선한다.
- 위쪽 기존 프롬프트에 남아 있을 수 있는 옛 ‘배경 세계관/서사 단계/관계성/2~4화 페이싱’ 값은 레거시 호환 정보일 뿐이다. 충돌하면 아래 V4.4.34 값만 따른다.
- 기존 V3.5 관계 태그는 V4.4.34 관계축과 중복되므로 이번 프롬프트에서는 비활성화했다.
...
===== END VELOUR V4.4.34 =====`;
  ...
  /* =========================================================
     V4.4.34 STAGE-ISOLATED PROMPT FIREWALL
...
- 사용자 스토리라인에 명시되지 않은 대규모 시간점프를 임의로 만들지 않는다.`;
  ...
  // V4.4.34 RESPONSE VAULT + TOKEN USAGE LEDGER.
  ...
  async function exportVelourResponseVault(){
    try{
      ...
      const payload={format:'VELOUR_RESPONSE_VAULT_V1',engine:'V4.4.34',exportedAt:new Date().toISOString(),responses,usage};
  ...
  function markGenerationOutcome(status,detail={}){
    window.__VELOUR_LAST_GENERATION_OUTCOME__=Object.assign({status,at:Date.now(),engine:'4.4.34'},detail||{});
    try{renderUsageSummary();}catch(e){}
    return window.__VELOUR_LAST_GENERATION_OUTCOME__;
  }


  // V4.4.34: CONFIRMED EPISODE LEDGER.
  ...
  function refreshReaderCharCounter(){
    ...
    counter.textContent=ok
      ? `본문 ${count.toLocaleString()}자 · 목표 충족 · ${modelLabelForCounter()} · ENGINE V4.4.34`
      : `본문 ${count.toLocaleString()}자 · 목표 ${target.toLocaleString()}자+보다 짧음 · ${modelLabelForCounter()} · ENGINE V4.4.34`;
  }
...
같은 EP에서 다시 생성해줘.`;
    ...
    const counter=document.getElementById('v35CharCount');
    if(counter){ counter.style.color='#ffd08a'; counter.textContent=`출력 미완료 · 독자 본문 ${len.toLocaleString()}자 · EP.${String(attemptedEp).padStart(2,'0')} 미확정 · ENGINE V4.4.34`; }
  }
...
EP.${attemptedEp}는 확정하지 않았고 에피소드/장기 메모리/임시저장/이어저장에 새 기록을 추가하지 않았어. 같은 번호로 다시 시도할 수 있어.${diagSummary}`;
    setUnconfirmedTitle(attemptedEp);
    const counter=document.getElementById('v35CharCount');
    if(counter){ counter.style.color='#ffd08a'; counter.textContent=`${kind.replaceAll('_',' ')} · EP.${String(attemptedEp).padStart(2,'0')} 미확정 · ENGINE V4.4.34`; }
  ...
  function hardContinuationReason(meta,isContinue,userInstruction=''){
    ...
    // V4.4.34: continuity META is an advisory self-review signal, not proof by itself.
  ...
  // V4.4.34: distinguish true HARD violations from advisory/self-review flags.
    ...
    window.generateStory=async function(isContinue=false){
      ...
      if(reason){
        ...
        if(!secondReason){
          ...
          console.info('VELOUR V4.4.34: advisory flags remain after retry; prose accepted with beat guard', secondReason);
          rememberConfirmedEpisode(secondEp,false);
          clearPendingRetryEpisode();
          ...
          console.warn('VELOUR V4.4.34: retry still has a HARD violation; output rejected', secondBlocking);
...
같은 EP에서 다시 생성해줘.`;
            ...
            const counter=document.getElementById('v35CharCount'); if(counter){counter.style.color='#ffd08a';counter.textContent=`HARD LOCK · EP.${String(attemptedEpisode).padStart(2,'0')} 미확정 · ENGINE V4.4.34`;}
          }
  ...
  /* =========================================================
     V4.4.10 STORAGE ENGINE (bundled into V4.4.34)
  ...
  async function migrateLegacyStorage(){
    ...
    const rec={key:'legacyMigrationV2',verified:!!verified,legacyCount:primary.length,idbCount:after.length,copied:restored,at:new Date().toISOString(),legacyPreserved:true,reconcileVersion:'4.4.34',restoredSources};
  ...
  async function recoverStoryLibraryIDB(){
    ...
    for(const c of missing){
      ...
      item.id=id; item.recoveredBy='V4.4.34'; item.recoveredFrom=c.source; item.recoveredAt=new Date().toISOString();
  ...
  console.info('✦ VELOUR Story Engine V4.4.34 Causal Buildup Lock + Response Vault loaded');
})();
