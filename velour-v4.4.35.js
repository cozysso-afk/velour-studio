...
/* =========================================================
   VELOUR STORY ENGINE V4.4.35 — STAGE-ISOLATED PROMPT + RESPONSE VAULT
...
(() => {
  ...
  window.__VELOUR_ENGINE_VERSION__ = '4.4.35';
  ...
  const MAX_SCENES = 12;
  // V4.4.35 three-tier longform memory.
  ...
  function installUI(){
    ...
    p.innerHTML=`
      <div class="v40-title"><b>✦ STORY ENGINE V4.4.35 · CAUSAL BUILDUP LOCK</b><span class="v40-badge">기존 저장함 호환</span></div>
      ...
      <details class="v40-section" id="v41SecPacing">
        ...
        <div class="v40-note">V4.4.35은 위 큰 구도 선택 안에서 <b>${POSITION_CATALOG.length}종 세부 체위/배치</b>를 자동 로테이션해. 매회 전체 목록을 프롬프트에 쏟지 않고, 최근 미사용·저사용 후보 소수만 골라 AI가 장소/감정선에 맞는 하나를 쓰게 해서 반복과 프롬프트 과밀을 같이 줄여.</div>
  ...
  function syncUI(full=true){
    ...
    const s=p.querySelector('#v4Status'); if(s){
      ...
      s.innerHTML=`<b>V4.4.35 BUILDUP LOCK ACTIVE</b> · ${esc(worldLabel())} · ${esc(relationshipLabel())} → ${esc(trajectoryLabel())}<br>`+
...
- 메타는 앱이 제거하므로 본문에 설명하거나 반복하지 않는다.`;
  ...
  function compactLegacyContext(text,isContinue=false){
    let out=String(text||'');
    // V3.5 and the pinned base each inject raw story history. V4.4.35 already supplies a compact
  ...
  if(typeof previousBuild==='function'){
    window.buildPrompt=function(isContinue=false){
      ...
      const prompt=`${base}\n\n===== VELOUR STORY ENGINE V4.4.35 · CAUSAL BUILDUP OVERRIDE =====
[우선순위] V4.4.35 HARD LOCK(금지/확정 캐논/현재 실행 단계) > 인과적 빌드업과 직전 확정 상태 > 사용자 현재 화 지시 > 취향·생활밀착·외형·친밀 장면 질감 > 기존 자동 디렉터.
- 설정 UI에서 OFF/금지로 둔 항목과 현재 CANON STORYLINE 단계는 단순 취향 제안이 아니라 HARD LOCK이다. 이번 화 추가 지시보다 우선한다.
- 위쪽 기존 프롬프트에 남아 있을 수 있는 옛 ‘배경 세계관/서사 단계/관계성/2~4화 페이싱’ 값은 레거시 호환 정보일 뿐이다. 충돌하면 아래 V4.4.35 값만 따른다.
- 기존 V3.5 관계 태그는 V4.4.35 관계축과 중복되므로 이번 프롬프트에서는 비활성화했다.
...
===== END VELOUR V4.4.35 =====`;
  ...
  /* =========================================================
     V4.4.35 STAGE-ISOLATED PROMPT FIREWALL
  ...
  function ageTimelineContinuityLock(raw){
    const s=String(raw||'');
    const hasChild=/(?:유년|어린\s*시절|어릴\s*때|초등학생|초등학교)/i.test(s);
    const hasMiddle=/(?:중학생|중학교)/i.test(s);
    const hasHigh=/(?:고등학생|고등학교)/i.test(s);
    const hasAdult=/(?:성인|대학생|대학교|직장인|현재)/i.test(s);
    const count=[hasChild,hasMiddle,hasHigh,hasAdult].filter(Boolean).length;
    if(count<2) return '';
    const stages=[];
    ...
    return `[AGE/TIMELINE CONTINUITY LOCK]
- 사용자 CANON에 존재하는 연령 단계의 시간축(${stages.join(' → ')})을 순서대로 유지한다.
- 안전 격리는 특정 민감 상세만 생략/치환하는 규칙이지, 해당 연령대의 사건·관계 변화·성장 단계를 삭제하거나 성인 시기로 점프하라는 지시가 아니다.
- 현재 스토리라인이 성인 이전 단계라면 그 단계의 비민감한 성장/감정/일상 사건을 충분히 전개한 뒤에만 다음 연령대로 이동한다.
- 사용자 스토리라인에 명시되지 않은 대규모 시간점프를 임의로 만들지 않는다.`;
  }
  ...
  function stageIsolatePrompt(text,ep){
    ...
    const ageLock=ageTimelineContinuityLock(text);
    let out=scrubMinorSexualHistory(text);
    if(phase.id!=='ADULT_DETAIL_ACTIVE') out=scrubInactiveAdultDetails(out,phase);
    if(ageLock) out=`${ageLock}\n\n${out}`;
    window.__VELOUR_PROMPT_PHASE__={...phase,beforeChars:String(text||'').length,afterChars:String(out||'').length,ageTimelineLocked:!!ageLock,at:new Date().toISOString()};
  ...
  // V4.4.35 RESPONSE VAULT + TOKEN USAGE LEDGER.
  ...
  async function exportVelourResponseVault(){
    try{
      ...
      const payload={format:'VELOUR_RESPONSE_VAULT_V1',engine:'V4.4.35',exportedAt:new Date().toISOString(),responses,usage};
  ...
  function markGenerationOutcome(status,detail={}){
    window.__VELOUR_LAST_GENERATION_OUTCOME__=Object.assign({status,at:Date.now(),engine:'4.4.35'},detail||{});
    try{renderUsageSummary();}catch(e){}
    return window.__VELOUR_LAST_GENERATION_OUTCOME__;
  }


  // V4.4.35: CONFIRMED EPISODE LEDGER.
  ...
  function refreshReaderCharCounter(){
    ...
    counter.textContent=ok
      ? `본문 ${count.toLocaleString()}자 · 목표 충족 · ${modelLabelForCounter()} · ENGINE V4.4.35`
      : `본문 ${count.toLocaleString()}자 · 목표 ${target.toLocaleString()}자+보다 짧음 · ${modelLabelForCounter()} · ENGINE V4.4.35`;
  }
  ...
  function likelyDiagnosticCause(kind,d){
    ...
    if(k==='HARD_LOCK') return 'Gemini 응답은 정상 수신됐지만 VELOUR 설정/진행 단계 검증에서 미확정 처리됨. V4.4.35는 자동 재생성을 하지 않는다.';
  ...
  function showHardLockReview(reason,attemptedEp,text=''){
    ...
    if(counter){
      ...
      counter.textContent=`⚠️ 설정 검토 필요 · 본문 ${len.toLocaleString()}자 · 자동 재생성 OFF · EP.${String(attemptedEp).padStart(2,'0')} 미확정 · ENGINE V4.4.35`;
    }
    const panel=document.getElementById('resultPanel');
    if(panel){
...
같은 EP에서 다시 생성해줘.`;
    setUnconfirmedTitle(attemptedEp);
    const counter=document.getElementById('v35CharCount');
    if(counter){ counter.style.color='#ffd08a'; counter.textContent=`출력 미완료 · 독자 본문 ${len.toLocaleString()}자 · EP.${String(attemptedEp).padStart(2,'0')} 미확정 · ENGINE V4.4.35`; }
  }
...
EP.${attemptedEp}는 확정하지 않았고 에피소드/장기 메모리/임시저장/이어저장에 새 기록을 추가하지 않았어. 같은 번호로 다시 시도할 수 있어.${diagSummary}`;
    setUnconfirmedTitle(attemptedEp);
    const counter=document.getElementById('v35CharCount');
    if(counter){ counter.style.color='#ffd08a'; counter.textContent=`${kind.replaceAll('_',' ')} · EP.${String(attemptedEp).padStart(2,'0')} 미확정 · ENGINE V4.4.35`; }
  ...
  function hardContinuationReason(meta,isContinue,userInstruction=''){
    ...
    // V4.4.35: continuity META is an advisory self-review signal, not proof by itself.
  ...
  // V4.4.35: distinguish true HARD violations from advisory/self-review flags.
  ...
  if(typeof previousGenerate==='function'){
    window.generateStory=async function(isContinue=false){
      ...
      if(reason){
        // V4.4.35: NEVER spend a second API call automatically. One tap = one Gemini request.
        ...
        if(!blocking){
          ...
          console.info('VELOUR V4.4.35: advisory flags detected; prose accepted without automatic repair retry', reason);
  ...
  /* =========================================================
     V4.4.10 STORAGE ENGINE (bundled into V4.4.35)
  ...
  async function migrateLegacyStorage(){
    ...
    const rec={key:'legacyMigrationV2',verified:!!verified,legacyCount:primary.length,idbCount:after.length,copied:restored,at:new Date().toISOString(),legacyPreserved:true,reconcileVersion:'4.4.35',restoredSources};
  ...
  async function recoverStoryLibraryIDB(){
    ...
    for(const c of missing){
      ...
      item.id=id; item.recoveredBy='V4.4.35'; item.recoveredFrom=c.source; item.recoveredAt=new Date().toISOString();
  ...
  console.info('✦ VELOUR Story Engine V4.4.35 Causal Buildup Lock + Response Vault loaded');
})();
