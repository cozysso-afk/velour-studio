...
(() => {
  ...
  function installPanel() {
    ...
    panel.innerHTML = `
      <span class="panel-tag">01-B. PACING, PROFILE & CROSSOVER · V3.5</span>
      <div class="v33-grid">
        <div class="form-row">
          <label>서사 속도</label>
          <select id="v33Pacing">
  ...
  function updateStatus() {
    ...
    el.innerHTML = `<b>V3.5 활성</b> · ${esc(pace)} · 자동연재 ${cfg.autoContinue?'ON':'OFF'} · 비주얼 ${cfg.defaultProfile?'ON':'OFF'} · 크로스오버 ${cross.length?cross.map(x=>esc(x[1])).join(' × '):'선택 없음'}`;
  }
...
===== END V3.5 =====`;
  ...
  badge.textContent = '✦ Story Engine V3.5 · Relationship Split + Auto Director + Detailed Visual Profile';
