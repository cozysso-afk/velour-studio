...
/* =========================================================
   VELOUR STORY ENGINE V4.2 — UNIFIED LONGFORM CANON / RELIGION + TAXONOMY PATCH
...
(() => {
  'use strict';
  if (window.__VELOUR_V42_INSTALLED__) return;
  window.__VELOUR_V42_INSTALLED__ = true;

  const CFG_KEY = 'VELOUR_STORY_ENGINE_V42';
  const OLD_CFG_KEYS = ['VELOUR_STORY_ENGINE_V41','VELOUR_STORY_ENGINE_V40'];
  ...
  function installUI(){
    ...
    p.innerHTML=`
      <div class="v40-title"><b>✦ STORY ENGINE V4.2 · RELIGION + TAXONOMY</b><span class="v40-badge">기존 저장함 호환</span></div>
  ...
  function bindUI(){
    ...
    const map={v4World:'world',v4Relationship:'relationship',v4Trajectory:'trajectory',v4HistoricalStyle:'historicalStyle',v4SocialA:'socialA',v4SocialB:'socialB',v4MilitaryStatus:'militaryStatus',v4ReligiousRule:'religiousRule',v4ReligiousNote:'religiousNote',v4HardCanon:'hardCanon',v4Storyline:'storyline',v4Pacing:'pacing',v4Unlock:'customUnlockEpisode',v4AdultFrequency:'adultFrequency',v4Cooldown:'cooldown',v4Variety:'variety',v4VarietyWindow:'varietyWindow',v4Dirty:'dirtyTalk',v4Profanity:'profanity',v4Insult:'insultMode'};
  ...
  function syncUI(full=true){
    ...
    const s=p.querySelector('#v4Status'); if(s){
      ...
      s.innerHTML=`<b>V4.2 ACTIVE</b> · ${esc(worldLabel())} · ${esc(relationshipLabel())} → ${esc(trajectoryLabel())}<br>`+
...
- 메타는 앱에서 자동 제거되므로 독자에게 설명하지 않는다.`;
  ...
  if(typeof previousBuild==='function'){
    window.buildPrompt=function(isContinue=false){
      ...
      return `${base}\n\n===== VELOUR STORY ENGINE V4.2 · UNIFIED OVERRIDE =====
[우선순위] 사용자 현재 입력 > V4.2 HARD CANON/스토리라인 잠금 > V4.2 장기 메모리 > 기존 자동 디렉터.
- 위쪽 기존 프롬프트에 남아 있을 수 있는 옛 ‘배경 세계관/서사 단계/관계성/2~4화 페이싱’ 값은 레거시 호환 정보일 뿐이다. 충돌하면 아래 V4.2 값만 따른다.
- 기존 V3.5 관계 태그는 V4.2 관계축과 중복되므로 이번 프롬프트에서는 비활성화했다.
...
===== END VELOUR V4.2 =====`;
  ...
  patchDraft();
  console.info('✦ VELOUR Story Engine V4.2 Religion + Taxonomy loaded');
})();
