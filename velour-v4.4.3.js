...
/* =========================================================
   VELOUR STORY ENGINE V4.4.3 — SLOWBURN EXPRESSION GATE PATCH
...
(() => {
  ...
  function installUI(){
    ...
    p.innerHTML=`
      <div class="v40-title"><b>✦ STORY ENGINE V4.4.3 · SLOWBURN GATED</b><span class="v40-badge">기존 저장함 호환</span></div>
      ...
      <details class="v40-section" id="v41SecPacing">
        ...
        <div class="v40-note">더티톡 수치는 ‘지금 당장 말하는 강도’가 아니라 <b>해금 후 최대 강도</b>로 적용해. AUTO에서는 첫 성인 장면 해금 화를 기준으로 내적 끌림 → 관심 → 플러팅 → 직접 성적 언어 순서로 늦춰.</div>
  ...
  function syncUI(full=true){
    ...
    const s=p.querySelector('#v4Status'); if(s){
      ...
      s.innerHTML=`<b>V4.4.3 ACTIVE</b> · ${esc(worldLabel())} · ${esc(relationshipLabel())} → ${esc(trajectoryLabel())}<br>`+
        `A ${esc(state.occupationA)} / B ${esc(state.occupationB)} · 첫 성인 장면 EP.${unlockEpisode()} · 성적 대사 ${state.sexualDialogueMode==='auto'?'AUTO':'CUSTOM'} · 쿨다운 ${Number(state.cooldown||0)}화<br>`+
...
- 더티톡 설정 최대 강도: ${dirtyMax}/100. 이것은 EP1부터 적용하는 현재 강도가 아니다.
- 현재 EP.${ep}의 외부 성적 대사 허용 단계: ${gate.allowedLevel}/4 · ${gate.label}.
${gate.allowedLevel<4?'- 현재는 더티톡 최대치가 높아도 그 수위를 선행 사용하지 않는다. 관계 단계가 해금될 때까지 대사를 억제한다.':'- 현재는 서사상 더티톡 해금 구간이다. 그래도 설정한 최대 강도 안에서 캐릭터와 상황에 맞게 사용한다.'}
- 욕설 강도: ${Number(state.profanity||0)}/100.${Number(state.profanity||0)>=10?' 혼잣말/감탄형 욕설은 상황에 맞을 때 허용한다.':' 욕설은 거의 사용하지 않는다.'}
...
- 직접 신체 호칭 허용 목록: ${allowed.length?allowed.join(', '):'없음 · 완곡 표현 우선'}. 단, 목록에서 켰더라도 현재 성적 대사 단계가 해금되기 전에는 대사에서 선행 사용하지 않는다.
...
${gate.allowedLevel<=1?'- 상대의 흥분 여부, 젖음/발기 등 성적 신체반응을 대사로 지적하거나 품평하지 않는다. 야한 농담·성적 명령·직접적인 욕망 고백도 금지한다.':''}
${gate.allowedLevel===2?'- 플러팅은 가능하지만 중의적·로맨틱 수준까지만. 상대의 성적 신체반응 직접 지적, 노골적 성행위 암시, 성적 명령, 더티톡은 금지한다.':''}
${professional?'- PROFESSIONAL BOUNDARY: 업무상 시선·포즈 지시·자세 교정·치료·상담·교육·경호·권한 관계에서 생기는 접촉은 성적 허가가 아니다. 초반에는 직업적으로 행동하고, 끌림이 생겨도 숨기거나 거리를 조절한다. 직업적 접촉을 빌미로 상대의 성적 반응을 지적하지 않는다.':''}
...
- sexualDialogueLevel: 0=성적 대사 없음, 1=개인적 관심/비성적 장난, 2=로맨틱 플러팅, 3=직접적인 성적 암시/신체반응 언급, 4=노골적 더티톡.
...
- 메타는 앱에서 자동 제거되므로 독자에게 설명하지 않는다.`;
  ...
  if(typeof previousBuild==='function'){
    window.buildPrompt=function(isContinue=false){
      ...
      return `${base}\n\n===== VELOUR STORY ENGINE V4.4.3 · SLOWBURN EXPRESSION OVERRIDE =====
[우선순위] 사용자 현재 입력 > V4.4 HARD CANON/스토리라인 잠금 > V4.4 외형·장기 메모리 > 기존 자동 디렉터.
- 위쪽 기존 프롬프트에 남아 있을 수 있는 옛 ‘배경 세계관/서사 단계/관계성/2~4화 페이싱’ 값은 레거시 호환 정보일 뿐이다. 충돌하면 아래 V4.4 값만 따른다.
- 기존 V3.5 관계 태그는 V4.4 관계축과 중복되므로 이번 프롬프트에서는 비활성화했다.
...
===== END VELOUR V4.4 =====`;
  ...
  if(typeof previousGenerate==='function'){
    window.generateStory=async function(isContinue=false){
      ...
      if(reason){
        ...
        if(!secondReason){ updateMemory(parsed.meta,secondEp); } else { console.warn('VELOUR V4.4.3: retry still violated pacing; memory not committed', secondReason); }
  ...
  patchDraft();
  console.info('✦ VELOUR Story Engine V4.4.3 Slowburn Expression Gate loaded');
})();
