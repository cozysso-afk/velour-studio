...
/* =========================================================
   VELOUR STORY ENGINE V4.4.4 — CLEAN PROMPT / REFUSAL HANDLING PATCH
...
- 메타는 앱에서 자동 제거되므로 독자에게 설명하지 않는다.`;
  ...
  if(typeof previousBuild==='function'){
    window.buildPrompt=function(isContinue=false){
      ...
      return `${base}\n\n===== VELOUR STORY ENGINE V4.4.4 · CLEAN LONGFORM OVERRIDE =====
...
[현재 문체 상한] ${currentIntensityMode().label}. 이 선택은 자동으로 R-15로 강등하지 않는다. 실제 표현 시점은 페이싱 게이트가 결정한다.
${canonDirective()}
${historicalDirective()}
${occupationDirective()}
${appearanceDirective()}
...
===== END VELOUR V4.4.4 =====`;
  ...
  function isLikelyModelRefusal(text){
    ...
    const refusal=/(진행할 수 없(?:습니다|어요)?|생성(?:이)? 불가능|생성할 수 없(?:습니다|어요)?|작성할 수 없(?:습니다|어요)?|제공할 수 없(?:습니다|어요)?|도와드릴 수 없(?:습니다|어요)?|요청하신 .*진행할 수 없|I (?:can(?:not|'t)|am unable to) (?:help|assist|provide|generate|write))/i;
  ...
  patchDraft();
  console.info('✦ VELOUR Story Engine V4.4.4 Clean Prompt + Refusal Handling loaded');
})();
