...
/* =========================================================
   VELOUR STORY ENGINE V4.4.36 — STAGE-ISOLATED PROMPT + RESPONSE VAULT
...
(() => {
  ...
  window.__VELOUR_ENGINE_VERSION__ = '4.4.36';
  ...
  const MAX_SCENES = 12;
  // V4.4.36 three-tier longform memory.
  ...
  function installUI(){
    ...
    p.innerHTML=`
      <div class="v40-title"><b>✦ STORY ENGINE V4.4.36 · CAUSAL BUILDUP LOCK</b><span class="v40-badge">기존 저장함 호환</span></div>
      ...
      <details class="v40-section" id="v41SecPacing">
        ...
        <div class="v40-note">V4.4.36은 위 큰 구도 선택 안에서 <b>${POSITION_CATALOG.length}종 세부 체위/배치</b>를 자동 로테이션해. 매회 전체 목록을 프롬프트에 쏟지 않고, 최근 미사용·저사용 후보 소수만 골라 AI가 장소/감정선에 맞는 하나를 쓰게 해서 반복과 프롬프트 과밀을 같이 줄여.</div>
  ...
  function syncUI(full=true){
    ...
    const s=p.querySelector('#v4Status'); if(s){
      ...
      s.innerHTML=`<b>V4.4.36 BUILDUP LOCK ACTIVE</b> · ${esc(worldLabel())} · ${esc(relationshipLabel())} → ${esc(trajectoryLabel())}<br>`+
...
- 메타는 앱이 제거하므로 본문에 설명하거나 반복하지 않는다.`;
  ...
  function compactLegacyContext(text,isContinue=false){
    let out=String(text||'');
    // V3.5 and the pinned base each inject raw story history. V4.4.36 already supplies a compact
  ...
  if(typeof previousBuild==='function'){
    window.buildPrompt=function(isContinue=false){
      ...
      const prompt=`${base}\n\n===== VELOUR STORY ENGINE V4.4.36 · CAUSAL BUILDUP OVERRIDE =====
[우선순위] V4.4.36 HARD LOCK(금지/확정 캐논/현재 실행 단계) > 인과적 빌드업과 직전 확정 상태 > 사용자 현재 화 지시 > 취향·생활밀착·외형·친밀 장면 질감 > 기존 자동 디렉터.
- 설정 UI에서 OFF/금지로 둔 항목과 현재 CANON STORYLINE 단계는 단순 취향 제안이 아니라 HARD LOCK이다. 이번 화 추가 지시보다 우선한다.
- 위쪽 기존 프롬프트에 남아 있을 수 있는 옛 ‘배경 세계관/서사 단계/관계성/2~4화 페이싱’ 값은 레거시 호환 정보일 뿐이다. 충돌하면 아래 V4.4.36 값만 따른다.
- 기존 V3.5 관계 태그는 V4.4.36 관계축과 중복되므로 이번 프롬프트에서는 비활성화했다.
...
===== END VELOUR V4.4.36 =====`;
  ...
  /* =========================================================
     V4.4.36 STAGE-ISOLATED PROMPT FIREWALL
  ...
  function promptStagePhase(ep){
    ...
    if(detailActive) return {id:'ADULT_DETAIL_ACTIVE',label:'현재 화 친밀 상세 활성',ep,gate:gate.allowedLevel,requested,beatDue,due:!!ps.due,overdue:!!ps.overdue};
    if(gate.allowedLevel>=4 || ps.unlocked || establishedSexualRelationship()) return {id:'UNLOCKED_IDLE',label:'해금됨 · 이번 화 상세 비활성',ep,gate:gate.allowedLevel,requested,beatDue,due:!!ps.due,overdue:!!ps.overdue};
    return {id:'PRE_UNLOCK',label:'빌드업 · 친밀 상세 격리',ep,gate:gate.allowedLevel,requested,beatDue,due:false,overdue:false};
  }
  ...
  function currentNarrativeStage(){
    ...
    // If current beat itself is not age-labelled, infer only from completed/current roadmap context,
    // never from future adult profile/occupation text.
    const completed=(active?.completed||[]).join('\n');
    const byCompleted=narrativeStageFromText(completed);
    if(byCompleted.id!=='unknown') return {...byCompleted,source:'completedBeat',currentBeat:current};
    const plot=String(document.getElementById('inputPlot')?.value||'');
    const byPlot=narrativeStageFromText(plot);
    if(byPlot.id!=='unknown') return {...byPlot,source:'plot',currentBeat:current};
  ...
  function safeMinorDevelopmentLine(row){
    ...
    return `- [연령 단계 유지 · ${label}] 이 시기는 삭제하거나 성인 시기로 건너뛰지 않는다. 서로의 사생활 경계, 외모·감정의 변화에 대한 어색함, 친밀감의 변화, 호기심·질투·거리감 같은 성장/관계 사건으로 이어간다. 민감한 행동·노출의 구체적 상세는 생략한다.`;
  }
...
`);
  ...
  function stageIsolatePrompt(text,ep){
    ...
    const ageLock=ageTimelineContinuityLock();
    const residenceLock=residenceCanonLock();
    const stage=currentNarrativeStage();
    let out=scrubMinorSexualHistory(text);
    ...
    if(locks) out=`${locks}\n\n${out}`;
    window.__VELOUR_PROMPT_PHASE__={...phase,beforeChars:String(text||'').length,afterChars:String(out||'').length,ageTimelineLocked:!!ageLock,residenceLocked:!!residenceLock,currentNarrativeStage:stage.id,currentNarrativeStageLabel:stage.label,residenceAnchor:residenceCanonAnchor(),at:new Date().toISOString()};
  ...
  // V4.4.36 RESPONSE VAULT + TOKEN USAGE LEDGER.
  ...
  async function exportVelourResponseVault(){
    try{
      ...
      const payload={format:'VELOUR_RESPONSE_VAULT_V1',engine:'V4.4.36',exportedAt:new Date().toISOString(),responses,usage};
  ...
  function markGenerationOutcome(status,detail={}){
    window.__VELOUR_LAST_GENERATION_OUTCOME__=Object.assign({status,at:Date.now(),engine:'4.4.36'},detail||{});
    try{renderUsageSummary();}catch(e){}
    return window.__VELOUR_LAST_GENERATION_OUTCOME__;
  }


  // V4.4.36: CONFIRMED EPISODE LEDGER.
  ...
  function refreshReaderCharCounter(){
    ...
    counter.textContent=ok
      ? `본문 ${count.toLocaleString()}자 · 목표 충족 · ${modelLabelForCounter()} · ENGINE V4.4.36`
      : `본문 ${count.toLocaleString()}자 · 목표 ${target.toLocaleString()}자+보다 짧음 · ${modelLabelForCounter()} · ENGINE V4.4.36`;
  }
  ...
  function likelyDiagnosticCause(kind,d){
    ...
    if(k==='HARD_LOCK') return 'Gemini 응답은 정상 수신됐지만 VELOUR 설정/진행 단계 검증에서 미확정 처리됨. V4.4.36는 자동 재생성을 하지 않는다.';
  ...
  function generationDiagnosticText(){
    ...
    const lines=[
      ...
      `Narrative stage lock: ${window.__VELOUR_PROMPT_PHASE__?.currentNarrativeStageLabel||'-'} · ${window.__VELOUR_PROMPT_PHASE__?.currentNarrativeStage||'-'}`,
  ...
  function showHardLockReview(reason,attemptedEp,text=''){
    ...
    if(counter){
      ...
      counter.textContent=`⚠️ 설정 검토 필요 · 본문 ${len.toLocaleString()}자 · 자동 재생성 OFF · EP.${String(attemptedEp).padStart(2,'0')} 미확정 · ENGINE V4.4.36`;
    }
    const panel=document.getElementById('resultPanel');
    if(panel){
...
같은 EP에서 다시 생성해줘.`;
    setUnconfirmedTitle(attemptedEp);
    const counter=document.getElementById('v35CharCount');
    if(counter){ counter.style.color='#ffd08a'; counter.textContent=`출력 미완료 · 독자 본문 ${len.toLocaleString()}자 · EP.${String(attemptedEp).padStart(2,'0')} 미확정 · ENGINE V4.4.36`; }
  }
...
EP.${attemptedEp}는 확정하지 않았고 에피소드/장기 메모리/임시저장/이어저장에 새 기록을 추가하지 않았어. 같은 번호로 다시 시도할 수 있어.${diagSummary}`;
    setUnconfirmedTitle(attemptedEp);
    const counter=document.getElementById('v35CharCount');
    if(counter){ counter.style.color='#ffd08a'; counter.textContent=`${kind.replaceAll('_',' ')} · EP.${String(attemptedEp).padStart(2,'0')} 미확정 · ENGINE V4.4.36`; }
  ...
  function hardContinuationReason(meta,isContinue,userInstruction=''){
    ...
    // V4.4.36: continuity META is an advisory self-review signal, not proof by itself.
  ...
  function userExplicitlyRequestsLifeStageJump(){
    ...
    return /(?:졸업|입학|대학생|대학교|대학\s*생활|성인\s*시기|몇\s*년\s*(?:후|뒤)|시간이\s*흐|성인이\s*된|스물(?:한)?\s*살)/i.test(`${u}\n${b}`);
  }
  ...
  function ageStageTextViolationReason(text){
    ...
    const t=String(text||'').replace(/\[\[VELOUR_V4_META\]\][\s\S]*$/,'');
    const adultNow=/(?:그는|그녀는|도진은|세아는|남주는|여주는)?[^\n.]{0,22}(?:대학생(?:이|으로|인)|대학교\s*\d+학년|대학\s*\d+학년|캠퍼스\s*생활|학과\s*(?:수업|과제|동기)|직장인(?:이|으로|인)|회사에\s*출근|21\s*세(?:가|인|로)?|스물(?:한)?\s*살(?:이|로|인)?)/i.test(t);
    if(adultNow) return `현재 CANON 연령 단계는 ${stage.label}인데 본문이 대학생/직장인 현재 시점으로 점프했다. 현재 학년·연령을 유지하고 성인 프로필은 미래 참고값으로만 둘 것.`;
    return '';
  }
  ...
  function residenceTextViolationReason(text){
    ...
    const t=String(text||'').replace(/\[\[VELOUR_V4_META\]\][\s\S]*$/,'');
    if(/(?:자취방|혼자\s*사는\s*(?:방|집)|자기\s*원룸|자신의\s*원룸|원룸으로\s*돌아|오피스텔로\s*돌아|기숙사\s*방으로\s*돌아)/i.test(t)){
      return `현재 거주 캐논(${anchor})과 달리 자취방/원룸/기숙사 거주를 새로 만들었다. 이사 사건이 없으므로 기존 부모님 집/본가 거주를 유지할 것.`;
    }
    return '';
  }
  ...
  // V4.4.36: distinguish true HARD violations from advisory/self-review flags.
  ...
  if(typeof previousGenerate==='function'){
    window.generateStory=async function(isContinue=false){
      ...
      if(reason){
        // V4.4.36: NEVER spend a second API call automatically. One tap = one Gemini request.
        ...
        if(!blocking){
          ...
          console.info('VELOUR V4.4.36: advisory flags detected; prose accepted without automatic repair retry', reason);
  ...
  /* =========================================================
     V4.4.10 STORAGE ENGINE (bundled into V4.4.36)
  ...
  async function migrateLegacyStorage(){
    ...
    const rec={key:'legacyMigrationV2',verified:!!verified,legacyCount:primary.length,idbCount:after.length,copied:restored,at:new Date().toISOString(),legacyPreserved:true,reconcileVersion:'4.4.36',restoredSources};
  ...
  async function recoverStoryLibraryIDB(){
    ...
    for(const c of missing){
      ...
      item.id=id; item.recoveredBy='V4.4.36'; item.recoveredFrom=c.source; item.recoveredAt=new Date().toISOString();
  ...
  console.info('✦ VELOUR Story Engine V4.4.36 Causal Buildup Lock + Response Vault loaded');
})();
