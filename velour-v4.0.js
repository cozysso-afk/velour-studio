...
/* =========================================================
   VELOUR STORY ENGINE V4.0 — LONGFORM CANON / MEMORY PATCH
   Loaded AFTER velour-v3.5.js.

   Goals
   - preserve V2 library/draft and V3.5 controls
...
(() => {
  'use strict';
  if (window.__VELOUR_V40_INSTALLED__) return;
  window.__VELOUR_V40_INSTALLED__ = true;

  const CFG_KEY = 'VELOUR_STORY_ENGINE_V40';
  ...
  const RELATIONSHIPS = [
    ...
    ['reunion','헤어진 뒤 재회'], ['dating','현재 연인'], ['married','부부'], ['blind_date','소개/맞선 상대'],
    ['contract','계약 관계'], ['fake_dating','가짜 연애'], ['secret_relation','비밀 관계'],
    ['fwb','섹파 / FWB'], ['fwb_repeat','원나잇 후 반복적으로 만나는 사이'],
  ...
  const OCCUPATIONS = {
    ...
    '작가·출판·웹툰': ['문학소설 작가','로맨스소설 작가','성인 로맨스 소설 작가','웹소설 작가','로맨스 웹소설 작가','19금 웹소설 작가','야설/에로틱 픽션 작가','BL 작가','로판 작가','시나리오 작가','드라마 작가','웹툰 스토리 작가','웹툰 작화가','성인 웹툰 스토리 작가','19금 웹툰 작화가','일러스트레이터','출판 편집자','웹소설 PD','웹툰 PD','콘텐츠 편집자','번역가'],
    ...
    '교육·연구': ['대학 교수','연구원','박사과정 연구자','학원강사','교사','외국어 강사','통역사','번역가','사서','큐레이터'],
  ...
  const DEFAULT = {
    world:'modern_general', relationship:'childhood', trajectory:'organic', historicalStyle:'readable',
    occCategoryA:'작가·출판·웹툰', occupationA:'웹소설 작가', occCategoryB:'기업·재계', occupationB:'회사원',
    socialA:'ordinary', socialB:'ordinary', militaryStatus:'',
  ...
  function load(){
    const saved=safeParse(localStorage.getItem(CFG_KEY)||'null',null);
  ...
  function installCss(){
    ...
    const st=document.createElement('style'); st.id='velour-v40-css'; st.textContent=`
      .v40-panel{border:1px solid rgba(245,196,107,.32);background:linear-gradient(180deg,rgba(28,8,17,.94),rgba(18,5,12,.92));border-radius:22px;padding:16px;margin:0 0 16px}
      .v40-title{display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:12px}.v40-title b{color:#ffebaa;font-size:13px}.v40-badge{font-size:9px;color:#bca7b2}
      ...
      details.v40-section summary{cursor:pointer;color:#f5c46b;font-weight:700;font-size:11.5px;margin-bottom:10px}
  ...
  function installUI(){
    ...
    const anchor=document.getElementById('velourV33Panel') || document.querySelector('.card-panel') || document.querySelector('.hero-banner');
    if(!anchor) return;
    const p=document.createElement('section'); p.id='velourV40Panel'; p.className='v40-panel';
    p.innerHTML=`
      <div class="v40-title"><b>✦ STORY ENGINE V4.0 · LONGFORM CANON</b><span class="v40-badge">기존 저장함 호환</span></div>

      <div class="v40-section">
        <h4>1. 세계관 · 관계 · 진행 방향</h4>
        <div class="v40-grid">
          ...
          <div class="v40-field"><label>시대극 문체</label><select id="v4HistoricalStyle">${selectOptions(HISTORICAL_STYLES,state.historicalStyle)}</select></div>
        </div>
      </div>
      ...
      <div class="v40-section">
        ...
        <div class="v40-field" style="margin-top:8px"><label>군인일 때 현재 상태 (선택)</label><input id="v4MilitaryStatus" placeholder="예: 휴가 중 / 훈련 중 / 파견 / 복귀 직전 / 전역 임박" value="${esc(state.militaryStatus)}"></div>
      </div>

      <div class="v40-section">
        <h4>3. HARD CANON · 스토리라인 잠금</h4>
        ...
        <div class="v40-note">각 단계는 여러 화에 걸쳐 확장 가능. 다음 단계 선행·건너뛰기는 금지. 모델이 단계 완료 메타를 정상 반환하면 자동으로 다음 단계로 넘어가고, 버튼으로 직접 보정할 수도 있어.</div>
      </div>

      <details class="v40-section" open>
      ...
      <details class="v40-section" open>
        <summary>5. LANGUAGE · 더티톡/욕설/직접 호칭 분리</summary>
        <div class="v40-grid">
        ...
        <div class="v40-note">더티톡 강도 ≠ 욕설 강도. 욕을 거의 안 쓰면서도 더티톡은 높게 설정 가능. 혼잣말/감탄형 ‘씨발’은 욕설 수치가 10 이상이면 허용하되 상대를 비하하는 ‘○○년’식 호칭은 기본 금지.</div>
      </details>
  ...
  function bindUI(){
    ...
    p.querySelector('#v4BeatNext')?.addEventListener('click',()=>{const beats=storylineBeats();state.beatIndex=Math.min(Math.max(0,beats.length-1),Number(state.beatIndex||0)+1);save(state);syncUI(false);});
  ...
  function syncUI(full=true){
    const p=document.getElementById('velourV40Panel'); if(!p)return;
    if(full){
      const values={v4World:state.world,v4Relationship:state.relationship,v4Trajectory:state.trajectory,v4HistoricalStyle:state.historicalStyle,v4OccCatA:state.occCategoryA,v4OccCatB:state.occCategoryB,v4OccA:state.occupationA,v4OccB:state.occupationB,v4SocialA:state.socialA,v4SocialB:state.socialB,v4MilitaryStatus:state.militaryStatus,v4HardCanon:state.hardCanon,v4Storyline:state.storyline,v4Pacing:state.pacing,v4Unlock:state.customUnlockEpisode,v4AdultFrequency:state.adultFrequency,v4Cooldown:state.cooldown,v4Variety:state.variety,v4VarietyWindow:state.varietyWindow,v4Dirty:state.dirtyTalk,v4Profanity:state.profanity,v4Insult:state.insultMode};
    ...
    const beats=storylineBeats(); const idx=Math.min(Number(state.beatIndex||0),Math.max(0,beats.length-1));
    const bs=p.querySelector('#v4BeatStatus'); if(bs)bs.textContent=beats.length?`현재 ${idx+1}/${beats.length} · ${beats[idx]}`:'스토리라인 미입력';
    const s=p.querySelector('#v4Status'); if(s){
      const last=state.runtime.scenes?.slice(-1)[0];
      s.innerHTML=`<b>V4 ACTIVE</b> · ${esc(worldLabel())} · ${esc(relationshipLabel())} → ${esc(trajectoryLabel())}<br>`+
...
- “야한 대사 = 욕설 많이 사용”으로 단순화하지 않는다.`;
  ...
  function canonDirective(){
    const beats=storylineBeats(); const idx=Math.min(Number(state.beatIndex||0),Math.max(0,beats.length-1));
    const current=beats[idx]||''; const future=beats.slice(idx+1);
    return `
...
${beats.length?`\n[CANON STORYLINE LOCK]\n현재 진행 가능한 단계 ${idx+1}/${beats.length}: ${current}\n- 이 단계는 여러 화로 충분히 늘려도 된다.\n- 현재 단계가 실제로 완료되기 전에는 다음 단계로 넘어가지 않는다.\n- 선행/건너뛰기/순서변경 금지.\n${future.length?`아직 금지된 미래 단계:\n${future.map((b,i)=>`${idx+i+2}. ${b}`).join('\n')}`:'현재가 마지막 단계다.'}`:''}`;
  ...
  function historicalDirective(){
    if(!String(state.world).startsWith('historical')) return '';
    return `
[시대극 문체 프리셋]
- ${histStyleLabel()}.
...
- 메타는 앱에서 자동 제거되므로 독자에게 설명하지 않는다.`;
  ...
  if(typeof previousBuild==='function'){
    window.buildPrompt=function(isContinue=false){
      const base=previousBuild(isContinue); const ep=epNumber();
      return `${base}\n\n===== VELOUR STORY ENGINE V4.0 · OVERRIDE =====
[우선순위] 사용자 현재 입력 > V4 HARD CANON/스토리라인 잠금 > V4 장기 메모리 > 기존 V3.5 자동 디렉터.
V3.5의 “2~4화 빌드업”처럼 더 짧은 규칙과 충돌하면 아래 V4 장기 규칙을 우선한다.
...
===== END VELOUR V4.0 =====`;
  ...
  function updateMemory(meta,ep){
    ...
    if(meta.beatComplete){ const beats=storylineBeats(); if(beats.length) state.beatIndex=Math.min(beats.length-1,Number(state.beatIndex||0)+1); }
  ...
  if(typeof previousGenerate==='function'){
    window.generateStory=async function(isContinue=false){
      ...
      stripMetaEverywhere(); updateMemory(parsed.meta,ep); patchDraft();
  ...
  function wrapLibraryPersistence(){
    ...
    if(typeof window.restoreStory==='function'&&!window.__VELOUR_V40_RESTORE_WRAP__){
      ...
      window.restoreStory=function(id){
        ...
        const out=old.apply(this,arguments); if(item?.v4State){state=Object.assign(clone(DEFAULT),item.v4State);state.terms=Object.assign({},DEFAULT.terms,item.v4State.terms||{});state.runtime=Object.assign({},DEFAULT.runtime,item.v4State.runtime||{});save(state);syncUI();} return out;
      };
    }
    if(typeof window.restoreDraftStory==='function'&&!window.__VELOUR_V40_DRAFT_WRAP__){
      ...
      window.restoreDraftStory=function(){
        ...
        const out=old.apply(this,arguments);if(d?.v4State){state=Object.assign(clone(DEFAULT),d.v4State);state.terms=Object.assign({},DEFAULT.terms,d.v4State.terms||{});state.runtime=Object.assign({},DEFAULT.runtime,d.v4State.runtime||{});save(state);syncUI();}return out;
      };
    }
  }
  ...
  patchDraft();
  console.info('✦ VELOUR Story Engine V4.0 loaded');
})();
