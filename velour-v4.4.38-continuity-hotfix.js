...
(() => {
  ...
  window.__VELOUR_CONTINUITY_COST_VERSION__ = '1.2.1';
  ...
  function settledCanonLines(state){
    ...
    const settled = /(?:하기로\s*(?:함|했다|했음|결정|합의)|하게\s*(?:됨|됐다|되었|되기로)|시작(?:함|했다|했음|하기로)|합의(?:함|했다|됨)?|결정(?:함|했다|됨)?|약속(?:함|했다|됨)?|계약(?:함|했다|체결)|맡기로|배우기로|가르치기로|사귀기로|동거하기로|과외하기로|연재하기로|근무하기로|연인(?:이|으로)?\s*(?:됨|됐다|되었|되기로)|애인(?:이|으로)?\s*(?:됨|됐다|되었|되기로)|커플(?:이|로)?\s*(?:됨|됐다|되었|되기로))/i;
  ...
  const RESULT_ENDING = '(?:된다|됨|됐다|되었다|되었)';
  const STATE_TRANSITION_CUE = /(?:(?:연인|애인|커플|남자친구|여자친구)(?:이|가|으로)?\s*(?:되(?:는|었다|었|기)?|된다|됨|됐다)|(?:사귀|연애)(?:기\s*시작|를\s*시작|하게\s*(?:된다|됨|됐다|되었다|되었))|(?:서로\s*)?(?:반말|존댓말|존대말|존대)(?:을|로)?\s*(?:하|쓰|바꾸|전환|하게\s*(?:된다|됨|됐다|되었다|되었))|(?:호칭|부르는\s*호칭)[^.!?]{0,28}(?:바꾸|정하|되|로\s*하)|(?:오빠|형|누나|언니|선배|후배|팀장님|대표님|선생님|이름)(?:이라고|라고|으로|로)\s*부르(?:기로|게\s*(?:된다|됨|됐다|되었다|되었))|(?:동거|같이\s*살|함께\s*살)(?:기\s*시작|을\s*시작|게\s*(?:된다|됨|됐다|되었다|되었)|기로)|(?:이사|거처|거주지|사는\s*곳)[^.!?]{0,28}(?:옮기|바꾸|정하|로\s*가|게\s*(?:된다|됨|됐다|되었다|되었))|(?:직업|직책|역할|신분|소속)[^.!?]{0,28}(?:바뀌|변경|되|맡|정해)|(?:팀장|상사|부하|비서|매니저|경호원|동료|동업자|교사|학생|선생|보호자)(?:이|가)?\s*(?:되|됐|된다|됨|맡게\s*(?:된다|됨|됐다|되었다|되었))|(?:비밀|정체|사실|과거|진실)[^.!?]{0,32}(?:알게\s*(?:된다|됨|됐다|되었다|되었)|알아내|알았다|공유하|밝히))/i;
  const ONGOING_DOMAIN = /(?:과외|수업|레슨|훈련|연습|상담|치료|프로젝트|연재|업무|근무|출근|동거|연애|사귀|연인|애인|커플|계약|경호|보호|돌봄|방문|연락|만남|호칭|반말|존댓말|존대|이사|거주|거처|직업|직책|역할|신분|소속|비밀|정체|진실|정기적으로)/i;
  const ONE_SHOT = /(?:이번\s*화|다음\s*화|오늘|내일|그날|이번만|한\s*번|1회|잠깐|잠시)/i;
  const END_CUE = /(?:그만두|그만하|중단|종료|끝내|취소|해지|관두|헤어지|이별|결별|더\s*이상[^.!?]{0,30}(?:않|안\s*하)|하지\s*않기로|안\s*하기로|쉬기로)/i;
  const CHANGE_CUE = /(?:변경|바꾸|조정|줄이|늘리|돌아가|주\s*\d+\s*회|월\s*\d+\s*회)/i;

  const TOPICS = [
    ['과외', /과외|개인\s*수업/], ['수업', /수업|레슨/], ['훈련', /훈련|연습/], ['상담', /상담/],
    ['치료', /치료/], ['프로젝트', /프로젝트|공동\s*작업/], ['연재', /연재/], ['근무', /근무|출근|직장/],
    ['동거', /동거|같이\s*살|함께\s*살/], ['연애', /연애|사귀|연인|애인|커플|남자친구|여자친구/], ['계약', /계약/], ['경호', /경호|보호/],
    ['돌봄', /돌봄|간병|보살피/], ['방문', /방문|찾아오|찾아가/], ['연락', /연락/], ['만남', /만남|만나기로/],
    ['호칭말투', /호칭|반말|존댓말|존대말|존대|오빠|형|누나|언니|선배|후배/],
    ['거주', /이사|거처|거주지|사는\s*곳/], ['직업역할', /직업|직책|역할|신분|소속|팀장|상사|부하|비서|매니저|경호원|동료|동업자|교사|학생|선생|보호자/],
    ['비밀인지', /비밀|정체|진실|과거|사실/]
  ];
  ...
  function isDurableUserClause(clause){
    ...
    const stateTransition = STATE_TRANSITION_CUE.test(text);
    const domain = ONGOING_DOMAIN.test(text);
    if (ONE_SHOT.test(text) && !persistent && !settled && !stateTransition) return false;
    if (END_CUE.test(text) && (domain || topicOf(text))) return true;
    return persistent || settled || stateTransition || (domain && /(?:하기로|하게\s*됐|하게\s*됨|시작)/.test(text));
  }
...
- ‘과외하기로 함/동거하기로 함/계약함/연인이 됨/호칭·말투가 바뀜/역할·거주·알게 된 비밀’처럼 이미 성립한 상태 전환은 다음 화에서 이전 상태로 되돌리지 않는다.
...
- ‘다음 화 추가 지시’는 기본적으로 1회성이다. 단, 그 화에서 확정된 관계 상태·호칭/존반말·직업/역할·거주/동거·알게 된 비밀/정체 같은 상태 전환은 확정 성공 뒤 장기 사실로 승격되어 이후 화에도 유지한다. 카페 방문·식사·산책·특정 장소 이동 같은 장면 지시는 별도 지속 표현이 없으면 1회성으로 끝낸다.
${settled.length ? `[현재 HARD CANON에서 ‘이미 성립한 상태’로 읽어야 할 항목]\n${settled.map(x => `- ${clean(x, 220)}`).join('\n')}` : '- HARD CANON에서 별도 추출된 완료형 상태 항목 없음.'}`;
  ...
  console.info('✦ VELOUR long-run continuity + prompt cost hotfix loaded · state transitions durable');
})();
