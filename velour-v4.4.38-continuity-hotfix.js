...
(() => {
  ...
  const STATE_TRANSITION_CUE = /(?:(?:연인|애인|커플|남자친구|여자친구)(?:이|가|으로)?\s*(?:되(?:는|었다|었|기)?|된다|됨|됐다)|(?:사귀|연애)(?:기\s*시작|를\s*시작|하게\s*되)|(?:서로\s*)?(?:반말|존댓말|존대말|존대)(?:을|로)?\s*(?:하|쓰|바꾸|전환|하게\s*되)|(?:호칭|부르는\s*호칭)[^.!?]{0,28}(?:바꾸|정하|되|로\s*하)|(?:오빠|형|누나|언니|선배|후배|팀장님|대표님|선생님|이름)(?:이라고|라고|으로|로)\s*부르(?:기로|게\s*되)|(?:동거|같이\s*살|함께\s*살)(?:기\s*시작|을\s*시작|게\s*되|기로)|(?:이사|거처|거주지|사는\s*곳)[^.!?]{0,28}(?:옮기|바꾸|정하|로\s*가|게\s*되)|(?:직업|직책|역할|신분|소속)[^.!?]{0,28}(?:바뀌|변경|되|맡|정해)|(?:팀장|상사|부하|비서|매니저|경호원|동료|동업자|교사|학생|선생|보호자)(?:이|가)?\s*(?:되|됐|된다|됨|맡게\s*되)|(?:비밀|정체|사실|과거|진실)[^.!?]{0,32}(?:알게\s*되|알아내|알았다|알게\s*됐|공유하|밝히))/i;
