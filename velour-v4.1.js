...
/* =========================================================
   VELOUR STORY ENGINE V4.1 — UNIFIED LONGFORM CANON / MEMORY PATCH
...
(() => {
  'use strict';
  if (window.__VELOUR_V41_INSTALLED__) return;
  window.__VELOUR_V41_INSTALLED__ = true;

  const CFG_KEY = 'VELOUR_STORY_ENGINE_V41';
  const OLD_CFG_KEY = 'VELOUR_STORY_ENGINE_V40';
  ...
  const DYNAMICS = [
    ...
    ['teacher_adult','성인 과외/교육 관계'], ['religious_conflict','종교·규율과 욕망의 충돌'],
  ...
  const DEFAULT = {
    world:'modern_general', relationship:'childhood', trajectory:'organic', dynamics:[], historicalStyle:'readable',
  ...
  function load(){
    const saved=safeParse(localStorage.getItem(CFG_KEY)||localStorage.getItem(OLD_CFG_KEY)||'null',null);
  ...
  function installUI(){
    ...
    p.innerHTML=`
      <div class="v40-title"><b>✦ STORY ENGINE V4.1 · UNIFIED LONGFORM</b><span class="v40-badge">기존 저장함 호환</span></div>
  ...
  function syncUI(full=true){
    ...
    const hist=p.querySelector('#v41HistoricalField'); if(hist) hist.style.display=String(state.world).startsWith('historical')?'block':'none';
    ...
    const s=p.querySelector('#v4Status'); if(s){
      ...
      s.innerHTML=`<b>V4.1 ACTIVE</b> · ${esc(worldLabel())} · ${esc(relationshipLabel())} → ${esc(trajectoryLabel())}<br>`+
...
- 메타는 앱에서 자동 제거되므로 독자에게 설명하지 않는다.`;
  ...
  if(typeof previousBuild==='function'){
    window.buildPrompt=function(isContinue=false){
      ...
      return `${base}\n\n===== VELOUR STORY ENGINE V4.1 · UNIFIED OVERRIDE =====
[우선순위] 사용자 현재 입력 > V4.1 HARD CANON/스토리라인 잠금 > V4.1 장기 메모리 > 기존 자동 디렉터.
- 위쪽 기존 프롬프트에 남아 있을 수 있는 옛 ‘배경 세계관/서사 단계/관계성/2~4화 페이싱’ 값은 레거시 호환 정보일 뿐이다. 충돌하면 아래 V4.1 값만 따른다.
- 기존 V3.5 관계 태그는 V4.1 관계축과 중복되므로 이번 프롬프트에서는 비활성화했다.
...
===== END VELOUR V4.1 =====`;
  ...
  patchDraft();
  console.info('✦ VELOUR Story Engine V4.1 Unified loaded');
})();
