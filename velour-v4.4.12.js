...
/* =========================================================
   VELOUR STORY ENGINE V4.4.12 — NON-PENETRATIVE PLAY ROTATION
...
(() => {
  ...
  const DEFAULT = {
    ...
    runtime:{timeline:[],openThreads:[],scenes:[],relationshipState:'',lastAdultEpisode:0,retryCount:0,positionUsage:{},lastSuggestedPositions:[],playUsage:{},lastSuggestedPlays:[]}
  };
  ...
  function installUI(){
    ...
    p.innerHTML=`
      <div class="v40-title"><b>✦ STORY ENGINE V4.4.12 · STORAGE + VARIETY + PLAY ROTATION</b><span class="v40-badge">기존 저장함 호환</span></div>
      ...
      <details class="v40-section" id="v41SecPacing">
        ...
        <div class="v40-note">V4.4.12는 위 큰 구도 선택 안에서 <b>${POSITION_CATALOG.length}종 세부 체위/배치</b>를 자동 로테이션해. 매회 전체 목록을 프롬프트에 쏟지 않고, 최근 미사용·저사용 후보 4개만 골라 AI가 장소/감정선에 맞는 하나를 쓰게 해서 반복과 프롬프트 과밀을 같이 줄여.</div>
  ...
  function syncUI(full=true){
    ...
    const s=p.querySelector('#v4Status'); if(s){
      ...
      s.innerHTML=`<b>V4.4.12 ACTIVE</b> · ${esc(worldLabel())} · ${esc(relationshipLabel())} → ${esc(trajectoryLabel())}<br>`+
...
${scenes.length?`최근 장면 지문:\n${scenes.map(s=>`- EP${s.episode||'?'} | 장소 ${s.location||'?'} | 목적 ${s.purpose||'?'} | 큰구도 ${s.pattern||'없음'} | 세부체위 ${s.position||'없음'} | 플레이 ${(Array.isArray(s.plays)&&s.plays.length)?s.plays.join('·'):'없음'} | 시작 ${s.initiation||'?'} | 주도 ${s.control||'?'} | 대화 ${s.dialogueTone||'?'} | 엔딩 ${s.ending||'?'}`).join('\n')}`:'최근 장면 지문: 없음.'}
...
- “술/집/묘한 분위기/직전에서 끊기” 같은 동일 구조를 이름만 바꿔 반복하지 않는다.`;
  ...
  function playRotationDirective(ep){
    ...
    return `- 비삽입/오럴 플레이는 체위와 별도 축으로 관리한다. 선택 풀 ${(state.adultPlayTypes||[]).length}종 중 저사용/미사용 후보 ${candidates.length}개: ${candidates.length?candidates.map(x=>`[${x.id}] ${x.label}`).join(' / '):'선택 없음'}.
- 최근 사용 플레이: ${recent.length?recent.join(', '):'기록 없음'}.
- 성인 장면이 실제로 필요할 때만 후보 중 현재 감정선과 장면에 맞는 것을 0~2개 자연스럽게 선택한다. 삽입 체위가 있다고 비삽입 플레이를 반드시 붙이지 않고, 반대로 비삽입 플레이만으로 장면을 구성해도 된다.
- 같은 플레이를 연속 기본값처럼 반복하지 말고 선택 풀 안에서 장기적으로 편중을 줄인다. 사용자가 이번 화에 특정 플레이를 직접 지시하면 그 지시가 최우선이다.`;
  ...
  function varietyDirective(ep){
    ...
    save(state);
    return `
[성인 장면 다양성 · V4.4.12 AUTO ROTATION]
...
- 이번 화에 성인 장면이 실제로 서사상 필요한 경우에만, 저사용/미사용 우선 후보 ${candidates.length}개 중 장소·신체조건·감정선에 자연스러운 하나를 선택: ${candidates.length?candidates.map(x=>`[${x.id}] ${x.label}`).join(' / '):'후보 없음'}.
- 후보가 현재 장면에 물리적으로 부자연스러우면 허용 풀 안의 다른 최근 미사용 체위로 바꿔도 된다. 성인 장면 자체가 필요 없는 화라면 후보를 억지로 사용하지 않는다.
...
[[VELOUR_V4_META]]{"beatComplete":false,"canonViolation":false,"storylineSkipped":false,"repeatRisk":"low","adultScene":false,"sexualDialogueLevel":0,"expressionViolation":false,"professionalBoundaryViolation":false,"timeline":"이번 화에서 새로 확정된 사실 1문장","openThreads":["남은 복선/약속/갈등"],"closedThreads":["이번 화에서 회수한 항목"],"location":"대표 장소","purpose":"이번 화 핵심 기능","pattern":"성인 장면이 있으면 큰 구도, 없으면 none","positionId":"위 AUTO ROTATION 후보/카탈로그의 id를 알면 id, 아니면 none","position":"성인 장면의 실제 세부 체위/배치 이름, 없으면 none","playIds":["사용한 비삽입/오럴 플레이 id, 없으면 빈 배열"],"plays":["사용한 비삽입/오럴 플레이 이름, 없으면 빈 배열"],"initiation":"누가/어떤 계기로 시작했는지 짧게","control":"주도권 구조 짧게","dialogueTone":"대화 톤 짧게","ending":"엔딩 방식","relationshipState":"현재 관계 상태 한 문장"}[[/VELOUR_V4_META]]
...
- 메타는 앱에서 자동 제거되므로 독자에게 설명하지 않는다.`;
  ...
  if(typeof previousBuild==='function'){
    window.buildPrompt=function(isContinue=false){
      ...
      return `${base}\n\n===== VELOUR STORY ENGINE V4.4.12 · STORAGE + VARIETY + PLAY ROTATION OVERRIDE =====
[우선순위] 사용자 현재 입력 > V4.4.12 HARD CANON/스토리라인 잠금 > V4.4.12 생활밀착/외형/장기 메모리 > 기존 자동 디렉터.
- 위쪽 기존 프롬프트에 남아 있을 수 있는 옛 ‘배경 세계관/서사 단계/관계성/2~4화 페이싱’ 값은 레거시 호환 정보일 뿐이다. 충돌하면 아래 V4.4.12 값만 따른다.
- 기존 V3.5 관계 태그는 V4.4.12 관계축과 중복되므로 이번 프롬프트에서는 비활성화했다.
...
===== END VELOUR V4.4.12 =====`;
  ...
  function extractMeta(text){
    ...
    if(start<0){
      let meta=null;
      const clean=source.replace(META_RE,(all,json)=>{ if(!meta){ try{ meta=JSON.parse(json.trim()); }catch(e){} } return ''; }).trim();
    ...
    const clean=(source.slice(0,start)+tail).trim();
    if(closeAt<0) partial=true;
    return {clean,meta,found:true,partial};
  }

  function stripMetaText(text){ return extractMeta(text).clean; }
  ...
  function updateMemory(meta,ep){
    ...
    state.runtime.scenes.push({episode:ep,location:String(meta.location||'').trim(),purpose:String(meta.purpose||'').trim(),pattern:String(meta.pattern||'none').trim(),positionId:canonicalPosId,position:canonicalPosName,playIds:canonicalPlayIds,plays:canonicalPlayNames,initiation:String(meta.initiation||'').trim(),control:String(meta.control||'').trim(),dialogueTone:String(meta.dialogueTone||'').trim(),ending:String(meta.ending||'').trim(),adultScene:!!meta.adultScene});
  ...
  if(typeof previousGenerate==='function'){
    window.generateStory=async function(isContinue=false){
      ...
      if(reason){
        ...
        if(!secondReason){ updateMemory(parsed.meta,secondEp); } else { console.warn('VELOUR V4.4.12: retry still violated pacing/variety; memory not committed', secondReason); }
  ...
  /* =========================================================
     V4.4.10 STORAGE ENGINE (bundled into V4.4.12)
  ...
  function currentTextForIDB(){
    ...
    if(!t||/^(?:⚠️ )?(?:MODEL REFUSAL|API FILTER BLOCK)/.test(t)||t.includes('응답이 생성되지 않았습니다')) return '';
    return t;
  }
  ...
  window.__VELOUR_STORAGE_QA__={idbOpen,idbGet,idbGetAll,idbPut,idbDelete,storageStories,saveDraftIDB,migrateLegacyStorage,positionCandidates,selectedPositionPool,playCandidates,prettyBytes,playCatalog:PLAY_CATALOG};
  ...
  stripMetaEverywhere();
  patchDraft();
  console.info('✦ VELOUR Story Engine V4.4.12 Storage + Variety + Play Rotation loaded');
})();
