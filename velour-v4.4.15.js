...
/* =========================================================
   VELOUR STORY ENGINE V4.4.15 — INTIMACY TEXTURE + APPEARANCE GUARD
...
(() => {
  ...
  function installUI(){
    ...
    p.innerHTML=`
      <div class="v40-title"><b>✦ STORY ENGINE V4.4.15 · INTIMACY TEXTURE + APPEARANCE GUARD</b><span class="v40-badge">기존 저장함 호환</span></div>
      ...
      <details class="v40-section" id="v41SecPacing">
        ...
        <div class="v40-note">V4.4.15는 위 큰 구도 선택 안에서 <b>${POSITION_CATALOG.length}종 세부 체위/배치</b>를 자동 로테이션해. 매회 전체 목록을 프롬프트에 쏟지 않고, 최근 미사용·저사용 후보 4개만 골라 AI가 장소/감정선에 맞는 하나를 쓰게 해서 반복과 프롬프트 과밀을 같이 줄여.</div>
        ...
        <div class="v40-note">기본값은 <b>키스 많음 + 애무 길게 + 본행위 중에도 애무를 계속</b>이야. 더티톡이 해금된 뒤에는 숫자 치수 대신 몸매에 대한 자연스러운 칭찬을 자주 섞어. 가벼운 스팽킹은 상호 동의가 분명한 성인 장면에서만 몇 차례 허용하고, 별도 지시 없이 강한 폭력/BDSM으로 확대하지 않아.</div>
      </details>
  ...
  function syncUI(full=true){
    ...
    const s=p.querySelector('#v4Status'); if(s){
      ...
      s.innerHTML=`<b>V4.4.15 ACTIVE</b> · ${esc(worldLabel())} · ${esc(relationshipLabel())} → ${esc(trajectoryLabel())}<br>`+
...
- ${detailRule}
${userBlocksAdultScene()?'- 사용자가 이번 화의 성인 장면을 명시적으로 미루거나 제외했다. 사용자 지시를 우선하고 자동 배정을 강제하지 않는다.':ps.cooldownLocked?`- 현재는 쿨다운 중이므로 새 성인 장면을 억지로 넣지 않는다.`:ps.overdue?'- 현재는 목표 간격을 넘긴 OVERDUE 구간이다. HARD CANON과 사용자 지시에 충돌하지 않는 한 이번 화에는 adultScene=true가 되는 완결된 성인 친밀 장면을 실제로 배치한다. 대화/업무만으로 또 미루지 않는다.':ps.due?'- 현재는 자동 관계 진전 배정 구간이다. HARD CANON과 사용자 지시에 충돌하지 않는 한 이번 화에는 adultScene=true가 되는 성인 친밀 장면을 실제로 배치한다. 단순 키스나 암시만으로 대체해 다시 미루지 않는다.':'- 현재는 반드시 성인 장면을 넣어야 하는 화는 아니다. 다만 해금된 관계를 초반 상태로 되돌리지 말고 이미 쌓인 친밀도는 유지한다.'}
...
${cooldownLocked?`- 쿨다운 잠금: 최근 성인 장면이 EP.${last}. EP.${last+cool}까지는 감정 후폭풍/생활 변화/사건/갈등/친밀감 재축적을 우선하며 새 성인 장면을 넣지 않는다.`:(postUnlockState(ep).due&&!userBlocksAdultScene()?'- 현재 자동 빈도 스케줄상 관계 진전 배정 화다. 사용자가 제외하지 않았다면 성인 장면을 실제로 발생시킨다.':'- 현재 쿨다운에 의해 자동 금지된 상태는 아니다. 성인 장면 여부는 자동 빈도 스케줄과 사용자 지시를 따른다.')}
...
- “술/집/묘한 분위기/직전에서 끊기” 같은 동일 구조를 이름만 바꿔 반복하지 않는다.`;
  ...
  function playRotationDirective(ep){
    ...
    const ps=postUnlockState(ep);
    const autoDue=ps.unlocked && !ps.cooldownLocked && ps.due && !userBlocksAdultScene();
...
- 같은 플레이를 연속 기본값처럼 반복하지 말고 선택 풀 안에서 장기적으로 편중을 줄인다. 사용자가 이번 화에 특정 플레이를 직접 지시하면 그 지시가 최우선이다.`;
  ...
  function varietyDirective(ep){
    ...
    const autoDue=ps.unlocked && !ps.cooldownLocked && ps.due && !blocked;
...
- 메타는 앱에서 자동 제거되므로 독자에게 설명하지 않는다. 내부 id/영문 키는 본문에 쓰지 않고 메타에도 필요하지 않다.`;
  ...
  if(typeof previousBuild==='function'){
    window.buildPrompt=function(isContinue=false){
      ...
      return `${base}\n\n===== VELOUR STORY ENGINE V4.4.15 · INTIMACY TEXTURE + APPEARANCE GUARD OVERRIDE =====
[우선순위] 사용자 현재 입력 > V4.4.15 HARD CANON/스토리라인 잠금 > V4.4.15 생활밀착/외형/친밀 장면 질감/장기 메모리 > 기존 자동 디렉터.
- 위쪽 기존 프롬프트에 남아 있을 수 있는 옛 ‘배경 세계관/서사 단계/관계성/2~4화 페이싱’ 값은 레거시 호환 정보일 뿐이다. 충돌하면 아래 V4.4.15 값만 따른다.
- 기존 V3.5 관계 태그는 V4.4.15 관계축과 중복되므로 이번 프롬프트에서는 비활성화했다.
...
===== END VELOUR V4.4.15 =====`;
  ...
  function retryReason(meta,text,ep){
    ...
    const ps=postUnlockState(ep);
    if(ps.unlocked && !ps.cooldownLocked && (ps.due||ps.overdue) && !userBlocksAdultScene() && !meta?.adultScene){
  ...
  if(typeof previousGenerate==='function'){
    window.generateStory=async function(isContinue=false){
      ...
      if(reason){
        ...
        if(!secondReason){ updateMemory(parsed.meta,secondEp); } else { console.warn('VELOUR V4.4.15: retry still violated pacing/variety/appearance; memory not committed', secondReason); }
  ...
  /* =========================================================
     V4.4.10 STORAGE ENGINE (bundled into V4.4.15)
  ...
  stripMetaEverywhere();
  patchDraft();
  console.info('✦ VELOUR Story Engine V4.4.15 Appearance Narration Guard loaded');
})();
