...
/* =========================================================
   VELOUR STORY ENGINE V4.4.23 — BODY PROSE VARIATION
...
(() => {
  ...
  window.__VELOUR_ENGINE_VERSION__ = '4.4.23';
  ...
  function installUI(){
    ...
    p.innerHTML=`
      <div class="v40-title"><b>✦ STORY ENGINE V4.4.23 · CAUSAL BUILDUP LOCK</b><span class="v40-badge">기존 저장함 호환</span></div>
      ...
      <details class="v40-section" id="v41SecPacing">
        ...
        <div class="v40-note">V4.4.23은 위 큰 구도 선택 안에서 <b>${POSITION_CATALOG.length}종 세부 체위/배치</b>를 자동 로테이션해. 매회 전체 목록을 프롬프트에 쏟지 않고, 최근 미사용·저사용 후보 4개만 골라 AI가 장소/감정선에 맞는 하나를 쓰게 해서 반복과 프롬프트 과밀을 같이 줄여.</div>
  ...
  function syncUI(full=true){
    ...
    const s=p.querySelector('#v4Status'); if(s){
      ...
      s.innerHTML=`<b>V4.4.23 BUILDUP LOCK ACTIVE</b> · ${esc(worldLabel())} · ${esc(relationshipLabel())} → ${esc(trajectoryLabel())}<br>`+
...
- openingBridge는 이어쓰기 첫 장면이 직전 화의 결과·감정·약속·행동·장소 중 하나를 자연스럽게 이어받으면 ok다. 같은 분/같은 장소에서 바로 재개할 필요는 없으며, 짧은 이동·귀가·다음날 전환이 명시되고 앞 화의 결과가 이어지면 ok다. 핵심 연결 사건을 통째로 생략하고 이미 진행 중인 상태에서 시작했을 때만 jumped다. 첫 화는 not_applicable.
- unauthorizedTimeJump는 사용자 지시 없이 큰 시간/장소 점프로 반드시 보여줘야 할 인과 사건을 통째로 생략했을 때만 true다. 평범한 귀가·취침·출근·다음날 같은 자연스러운 전환을 한두 비트로 연결했다면 false다. startsMidEvent는 독자가 보지 못한 핵심 사건이 이미 진행 중인 상태에서 갑자기 시작했을 때만 true이며, 첫 문단들이 즉시 원인과 경위를 보여주면 false다.
...
- 메타는 앱에서 자동 제거되므로 독자에게 설명하지 않는다. 내부 id/영문 키는 본문에 쓰지 않고 메타에도 필요하지 않다.`;
  ...
  if(typeof previousBuild==='function'){
    window.buildPrompt=function(isContinue=false){
      ...
      return `${base}\n\n===== VELOUR STORY ENGINE V4.4.23 · CAUSAL BUILDUP OVERRIDE =====
[우선순위] V4.4.23 HARD LOCK(금지/확정 캐논/현재 실행 단계) > 인과적 빌드업과 직전 확정 상태 > 사용자 현재 화 지시 > 취향·생활밀착·외형·친밀 장면 질감 > 기존 자동 디렉터.
- 설정 UI에서 OFF/금지로 둔 항목과 현재 CANON STORYLINE 단계는 단순 취향 제안이 아니라 HARD LOCK이다. 이번 화 추가 지시보다 우선한다.
- 위쪽 기존 프롬프트에 남아 있을 수 있는 옛 ‘배경 세계관/서사 단계/관계성/2~4화 페이싱’ 값은 레거시 호환 정보일 뿐이다. 충돌하면 아래 V4.4.23 값만 따른다.
- 기존 V3.5 관계 태그는 V4.4.23 관계축과 중복되므로 이번 프롬프트에서는 비활성화했다.
...
===== END VELOUR V4.4.23 =====`;
    ...
    window.fetch=async function(input,init){
      ...
      try{
        ...
        if(isGemini){
          ...
          }catch(e){ window.__VELOUR_LAST_GEMINI_DIAGNOSTIC__={httpStatus:Number(res.status||0),httpOk:!!res.ok,parseError:true,at:Date.now()}; }
        }
        return res;
      }catch(err){
        if(isGemini) window.__VELOUR_LAST_GEMINI_DIAGNOSTIC__={networkError:String(err?.message||err||'NETWORK_ERROR'),at:Date.now()};
        throw err;
      }
    };
  }
  ...
  function markGenerationOutcome(status,detail={}){
    window.__VELOUR_LAST_GENERATION_OUTCOME__=Object.assign({status,at:Date.now(),engine:'4.4.23'},detail||{});
    return window.__VELOUR_LAST_GENERATION_OUTCOME__;
  }


  // V4.4.23: failed generation must retry the SAME episode number.
  // The legacy generator increments episodeCount at the start of a continuation,
  // so we keep the failed attempted episode as a pending transaction and force
  // the mutable base counter back to (attemptedEpisode - 1) before every retry.
  function pendingRetryEpisode(){
    const outcome=window.__VELOUR_LAST_GENERATION_OUTCOME__||{};
    const fromOutcome=outcome.status==='failed'?Number(outcome.attemptedEpisode||0):0;
    const fromWindow=Number(window.__VELOUR_PENDING_RETRY_EPISODE__||0);
    const fromState=Number(state?.runtime?.pendingRetryEpisode||0);
    return [fromOutcome,fromWindow,fromState].find(n=>Number.isFinite(n)&&n>0)||0;
  }
  ...
  function forceCounterForPendingRetry(){
    const n=pendingRetryEpisode();
    if(!n) return 0;
    try{ episodeCount=Math.max(0,n-1); }catch(e){}
    return n;
  }

  function forceCounterAfterFailure(attemptedEp){
    const n=Number(attemptedEp||pendingRetryEpisode()||0);
    if(!Number.isFinite(n)||n<=0) return;
    try{ episodeCount=Math.max(0,n-1); }catch(e){}
    rememberPendingRetryEpisode(n);
  }
  ...
  function refreshReaderCharCounter(){
    ...
    counter.textContent=ok
      ? `본문 ${count.toLocaleString()}자 · 목표 충족 · ${modelLabelForCounter()} · ENGINE V4.4.23`
      : `본문 ${count.toLocaleString()}자 · 목표 ${target.toLocaleString()}자+보다 짧음 · ${modelLabelForCounter()} · ENGINE V4.4.23`;
  }
...
같은 EP에서 다시 생성해줘.`;
    ...
    const counter=document.getElementById('v35CharCount');
    if(counter){ counter.style.color='#ffd08a'; counter.textContent=`출력 미완료 · 독자 본문 ${len.toLocaleString()}자 · EP.${String(attemptedEp).padStart(2,'0')} 미확정 · ENGINE V4.4.23`; }
  }
...
${copy[1]}
EP.${attemptedEp}는 확정하지 않았고 에피소드/장기 메모리/임시저장/이어저장에 새 기록을 추가하지 않았어. 같은 번호로 다시 시도할 수 있어.`;
    setUnconfirmedTitle(attemptedEp);
    const counter=document.getElementById('v35CharCount');
    if(counter){ counter.style.color='#ffd08a'; counter.textContent=`${kind.replaceAll('_',' ')} · EP.${String(attemptedEp).padStart(2,'0')} 미확정 · ENGINE V4.4.23`; }
  }
  ...
  function hardContinuationReason(meta,isContinue,userInstruction=''){
    ...
    // V4.4.23: continuity META is an advisory self-review signal, not proof by itself.
  ...
  // V4.4.23: distinguish true HARD violations from advisory/self-review flags.
  ...
  if(typeof previousGenerate==='function'){
    window.generateStory=async function(isContinue=false){
      if(isContinue) forceCounterForPendingRetry();
      else clearPendingRetryEpisode();
      const beforeEpisode=(()=>{try{return Number(episodeCount||1)}catch(e){return 1}})();
      const retryEpisode=isContinue?pendingRetryEpisode():0;
      const attemptedEpisode=isContinue?(retryEpisode||beforeEpisode+1):1;
      ...
      if(failure){
        rollbackBase();
        forceCounterAfterFailure(attemptedEpisode);
        state.runtime.retryDirective=''; state.runtime.retryCount=0; save(state);
      ...
      if(reason){
        ...
        if(!secondReason){
          ...
          state.runtime.retryDirective=''; save(state); patchDraft(); refreshReaderCharCounter(); markGenerationOutcome('committed',{episode:secondEp,attemptedEpisode}); syncUI(false);
          ...
          console.info('VELOUR V4.4.23: advisory flags remain after retry; prose accepted with beat guard', secondReason);
          clearPendingRetryEpisode();
          updateMemory(guarded,secondEp);
          state.runtime.retryDirective=''; state.runtime.retryCount=0; save(state); patchDraft(); refreshReaderCharCounter(); markGenerationOutcome('committed',{episode:secondEp,attemptedEpisode,advisory:true}); syncUI(false);
        } else {
          console.warn('VELOUR V4.4.23: retry still has a HARD violation; output rejected', secondBlocking);
...
같은 EP에서 다시 생성해줘.`;
            ...
            const counter=document.getElementById('v35CharCount'); if(counter){counter.style.color='#ffd08a';counter.textContent=`HARD LOCK · EP.${String(attemptedEpisode).padStart(2,'0')} 미확정 · ENGINE V4.4.23`;}
          }
        ...
        clearPendingRetryEpisode();
        updateMemory(parsed.meta,ep); patchDraft(); refreshReaderCharCounter();
        markGenerationOutcome('committed',{episode:ep,attemptedEpisode});
      }
    };
  }
  ...
  /* =========================================================
     V4.4.10 STORAGE ENGINE (bundled into V4.4.23)
  ...
  window.__VELOUR_STORAGE_QA__={idbOpen,idbGet,idbGetAll,idbPut,idbDelete,storageStories,saveDraftIDB,migrateLegacyStorage,positionCandidates,selectedPositionPool,playCandidates,prettyBytes,playCatalog:PLAY_CATALOG,stripPlannerArtifacts,postUnlockState,userBlocksAdultScene,appearanceMeasurementLeakReason,softenLeakedBodySpecs,userRequestsExactBodySpecs,repeatedBodyPhraseReason,bodyDescriptionDirective,bodyIntegrityReason,bodyLengthAdvisoryReason,readerBodyLength,generationFailureKind,isFailureScreenText,markGenerationOutcome,pendingRetryEpisode,rememberPendingRetryEpisode,clearPendingRetryEpisode,forceCounterForPendingRetry,forceCounterAfterFailure,updateMemory};
    ...
    if(typeof generated==='function'&&!window.__VELOUR_IDB_GENERATE_WRAP__){
      ...
      window.generateStory=async function(isContinue=false){
        ...
        try{ out=await generated.apply(this,arguments); }
          ...
          const attempted=pendingRetryEpisode()||Number((window.__VELOUR_LAST_GENERATION_OUTCOME__||{}).attemptedEpisode||0);
          if(attempted) forceCounterAfterFailure(attempted);
          markGenerationOutcome('failed',{kind:'WRAPPER_ERROR',detail:String(err?.message||err),attemptedEpisode:attempted||undefined});
          updateStorageSaveButton(); throw err;
        }
        const outcome=window.__VELOUR_LAST_GENERATION_OUTCOME__||{};
  ...
  stripMetaEverywhere();
  patchDraft();
  console.info('✦ VELOUR Story Engine V4.4.23 Causal Buildup Lock loaded');
})();
